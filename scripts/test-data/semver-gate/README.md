...
| `broken-dep.out` | 101 | **Abridged capture.** `bash scripts/check_semver.sh --crate trusty-common` on 2026-09-03, where the scratch resolution took `tinyvec` 1.13.0 — a release that calls `vec![]` while importing only the `alloc::vec` module, so it fails for every consumer of its `alloc` feature. This workspace pins 1.11.0 and builds clean, which is why the failure read as unreproducible (#6740). The only edit beyond the path rewrites: `Checking`/`Compiling` progress lines for packages other than `tinyvec` are replaced by a counted `[N unrelated … elided]` marker, because the capture ran to 450 lines of them. Every diagnostic line is verbatim. |
