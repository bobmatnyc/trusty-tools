...
| `break-lints.out` | 1 | `--crate trusty-mpm` on `main` at `2fcade761`, 2026-09-23: the real 1.6.3 -> 1.6.4 break, `7 fail, 1 warn`, with every `--- failure <lint>:` block and its `Failed in:` entries. The accepted-breaks cases (a)-(g) match declarations against it, so it has to be a real capture: the entry wording varies by lint. |
