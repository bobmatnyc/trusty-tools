...
reproduce it on `origin/main`, or, **only when the failing crate depends on
nothing you changed**, show your diff touches zero files in it
(`git diff --name-only origin/main...HEAD -- <crate>/`). For the
known-environmental flaky tests on this machine (the `trusty-search`
