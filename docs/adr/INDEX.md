...
| [0025](./0025-collapse-agent-and-skill-tier-hierarchies.md) | Collapse the agent and skill tier hierarchies: one deploy target, many declared sources, precedence resolved at deploy time | Proposed | One global canonical agent deploy target (no project-scoped agents); skills stay per-worktree; assets come from declared, numerically-prioritised sources with hard-error tie handling and four-surface override visibility; `~/.claude/skills` is never written to, read from, swept, or reported | crates `trusty-mpm`, `trusty-agents-common` |
