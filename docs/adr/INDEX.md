...
This index is the **single source of truth** for the ADR corpus. It serves as a quick reference for **consistency vetting** (see DOC-31 §3, "Related Decisions" protocol) and as a discoverability surface for understanding the architectural decision landscape.
...
| [0004](./0004-three-harnesses-shared-event-driven-common.md) | Three distinct harnesses on a shared event-driven trusty-common foundation | Proposed | Three independent harnesses (trusty-agents, trusty-search, trusty-memory) share event-driven, KG-backed trusty-common | Workspace |
...
| [0006](./0006-trusty-controller-naming.md) | Name the stack control plane `trusty-controller` (binary `tctl`) | Accepted | Control plane service is called trusty-controller; CLI binary is `tctl` (replaced by trusty-installer in ADR-0013) | Workspace |
...
| [0010](./0010-kg-edge-kind-extensibility.md) | KG edge-kind extensibility: first-class data-flow variants + Custom escape hatch | Proposed | KG supports standard edge kinds (depends, defines, calls, …) plus Custom for user-defined variants | Workspace |
| [0011](./0011-tctl-owns-service-lifecycle.md) | `tctl` headless control plane (owns boot/lifecycle); `trusty-console` is the single HTTP surface | Accepted | tctl manages daemon lifecycle (start/stop/status); trusty-console is the single HTTP API surface for all user interactions | Workspace |
...
- **ADRs 0001–0013** were grandfathered as Accepted without re-vetting when DOC-31 (this spec) was introduced.
- **Future ADRs** (0014+) must include a "Related Decisions" section (DOC-31 §3) documenting consistency vetting before acceptance.
