...
| [0048](0048-dispatched-writers-get-a-worktree-and-the-write-boundary-is-enforced.md) | Dispatched writers get a worktree; the write boundary is enforced | Amended by 0049, 0051 | A dispatched writer in a main checkout is granted harness isolation, source writes and commits there are denied for the PM and every agent, and the shared-writer query is keyed by directory rather than session. | `trusty-mpm` |
...
| [0051](0051-a-project-may-exempt-dispatched-agents-from-the-worktree-grant.md) | A project may exempt dispatched agents from the worktree grant | Accepted | A committed `dispatch_isolation = "main-checkout"` suppresses ADR-0048 decision 1's grant for that project so agents work in the checkout; the read fails closed, so an absent, unreadable, malformed, or unrecognised declaration keeps isolation on. | `trusty-mpm` |
