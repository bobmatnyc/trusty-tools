...
- **Sentinel-before-record creation race, closed by a grace window.** In BOTH real provisioning paths, the on-disk sentinel is written naming the new session as owner BEFORE that session's `SessionRecord` is persisted to the store — real I/O (git clone/`worktree add`, `prepare_session`) runs in between (`spawn_managed_cloned`: `provision_in` writes the sentinel, `create_with_id` persists the record afterward; `spawn_managed_inproject`: `reserve_inproject_worktree` writes the sentinel, `write_task_md`/`prepare_inproject_session` run, THEN `create_with_reserved_name` persists). A `SessionManager::get` lookup for that owner during this window returns `SessionNotFound` — indistinguishable, to a plain store lookup, from a session that legitimately existed and was later deleted. Without a mitigation, an orphan-GC sweep landing in that window would treat the brand-new, mid-creation worktree as provably ownerless and reclaim it: the SAME class of failure as the pre-#3649 incident in "Context" above (an unattributed worktree deleted out from under active use), just reached via a new window this ownership scheme itself introduces rather than the total absence of one. Caught in adversarial code review before merge, not observed in production. The fix is `worktree_ownership::OWNERLESS_GRACE` (10 minutes, self-contained — deliberately not derived from the daemon's configurable `ORPHAN_GC_INTERVAL_SECS`, which this module has no business depending on) plus `SessionManager::resolve_ownerless_with_grace`: a sentinel-named owner with NO resolvable record is ownerless only once the sentinel is OLDER than the grace window; a FOUND owner in a terminal state is ownerless regardless of age (its existence is direct proof the session ran to completion, never a mid-creation artifact). This closes the safety hole structurally, independent of either provisioning path's internal ordering. A more invasive fix — persisting a placeholder `SessionRecord` (state `Provisioning`) BEFORE the sentinel/worktree I/O, shrinking the race window itself rather than papering over it with a grace period — is deliberately deferred; see "Neutral / Follow-up work" below. Separately, and much more narrowly: `set_worktree_owner`'s registry-field write (as opposed to the sentinel) still lags record creation by a few lines (the post-creation-setter trade-off described in earlier drafts of this ADR) — that lag is benign because `known_owner_of` falls back to the sentinel whenever the registry field is unset, and the sentinel is what the grace window above protects.
...
- Shrink the sentinel-before-record creation race at its source: persist a placeholder `SessionRecord` (state `Provisioning`) BEFORE the sentinel/worktree I/O in both `spawn_managed_cloned` and `spawn_managed_inproject`, so a `SessionManager::get` for the owner succeeds (as `Provisioning`, correctly non-ownerless) for the ENTIRE provisioning duration rather than relying on the grace window's fixed 10-minute margin. Deferred from this PR: reordering either spawn path touches its rollback/error-handling semantics (the existing `TmuxSessionGuard`/orphan-reap-on-failure logic is tuned around the current ordering) and is a larger, separately-reviewable change; the grace window alone already closes the safety hole for any provisioning run that completes within it.
