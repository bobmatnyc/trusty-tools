...
2. **Additive eventstream mirror.** `handlers::record_listener_event` (new) appends a `StoredEvent` (`provider: "slack"`, `listener_id: "slack"`, `event_type: "message.<channel_type>"` e.g. `message.im`/`message.mpim`) to the SAME `EventStore` Gmail uses, then consults `EventStore::is_event_type_included` and publishes `Event::ListenerEventReceived` — reusing the identical generic SSE variant Gmail's poll loop emits, so the Events pane needs zero Slack-specific rendering code. This runs BEFORE the RBAC/dispatch branch in `handle_message`, so it captures every message in a paired channel regardless of whether the sender is a known RBAC user. It is dispatched via `tokio::spawn` (owned args, detached — the same pattern the pre-existing `relay_event` mirror below it already uses) so a slow store append never delays the Slack reply, and an append failure is logged and NOT fatal to the rest of the function — `is_event_type_included`/`publish` still run on the SAME event, matching `listeners::poll::poll_once`'s own log-and-continue posture on the identical failure (`listeners/poll.rs:326-331`), so a transient disk hiccup degrades durability, never live Events-pane visibility.
...
- The eventstream mirror is fully off the Slack reply's critical path: `tokio::spawn`-detached, and a store-append failure (e.g. a transient disk error) degrades ONLY the durable `events.jsonl` log — the live SSE mirror still fires — so a persistence hiccup can never make the bot appear slower to reply, and never silently blinds the Events pane either (code-critic-caught fix; see `record_listener_event`'s doc comment and `slack_listener_event_publishes_even_when_append_fails`).
...
- **The eventstream mirror can now observably lag or interleave with the Slack reply** (a consequence of detaching it via `tokio::spawn` rather than awaiting it inline): the Events pane may show a message a few milliseconds after — in rare scheduling cases, technically after — the bot's own reply lands in Slack, rather than strictly before. Acceptable because the mirror is telemetry, not a gate on the reply; nothing consumes it synchronously today (see "No wake binding" above).
