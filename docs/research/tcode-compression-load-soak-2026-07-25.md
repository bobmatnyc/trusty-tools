...
The payload text itself is **not** a single repeated character (the original soak's `"x".repeat(8000)` approach) — `synthetic_tool_output()` interleaves six realistic line shapes (unified-diff hunks, `grep path:line:match` rows, `cargo test FAILED`/panic/assertion lines) cycling until the target size is reached, so the token-per-byte character the crate's `estimate_tokens` (chars/4) heuristic sees is closer to what a real diff/log actually produces than a maximally-repetitive blob would be.
...
2. **`session.get_transcript`** — must return without an RPC error, with `compaction_events == 0` (the `TranscriptRecord`'s own durable threshold-compaction counter — a THIRD independent corroboration of the JSONL's `tcode-threshold` count, alongside the JSONL itself and `session.get_context_budget`'s `lifetime_compaction_alarm_count`).
...
- **Session fidelity: PASS.** `goals_after_soak: []` (all three touched slots clean), `transcript_compaction_events: 0` (third independent corroboration of zero fallback-compaction fires), `resume_after_soak_status: "finished"` (the session successfully ran one more full task after 245 turns of sustained heavy compaction pressure).
...
To reproduce the exploratory FAIL run, temporarily edit `LOAD_PAYLOAD_BYTES` in
`crates/trusty-code/src/task/mock_llm_soak_load.rs` to `[220_000, 300_000, 400_000]`,
rebuild, and re-run with `--calls 20`.
