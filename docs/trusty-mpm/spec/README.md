...
| **[SESSION_MANAGER_DRIVER_AGENT.md](./SESSION_MANAGER_DRIVER_AGENT.md)** | The harness-operator agent + session-manager-driver skill: how a calling agentic process drives trusty-mpm end-to-end (catalog sync → spawn → poll → inject/answer → lifecycle), the autonomy operating model sketch, the interface table, and the definition of done. |
| **[AUTONOMY_POLICY.md](./AUTONOMY_POLICY.md)** | The *implemented* driver autonomy policy: the T1–T4 tier model, the structured non-LLM guardrails (trusty-review / CI / search / memory consistency), and the session↔artifact correlation that bounds a session's scope. Maps to `crates/trusty-mpm/src/driver/`. |
