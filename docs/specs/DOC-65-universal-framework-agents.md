...
| `ticketing` | Issue operations: promote, search/deduplicate, create, update, close/reopen, label, milestone, link, triage, and comment | sonnet | Scope validation and workflow-state intelligence require judgment, not just mechanical repository operations | Owns issue artifacts only, following `tm-ticketing`; never performs git operations or creates/edits/merges PRs (→ `version-control`) | `crates/trusty-agents-common/src/assets/agents/ticketing.md`, `extends: base-agent` |
| `version-control` | All git and PR operations: branches, commits, rebases, pushes, tags, PR creation/update/merge, title, and body | sonnet | Owner ruling 2026-09-09 (#7274). A PR's labels, project and milestone are derived, not dictated: the component labels come from the crates its own diff touches, and the project and milestone come from the issue its `Refs #N` names — the same judgment that puts `ticketing` at sonnet | Never does issue bookkeeping (→ `ticketing`) or defines delivery policy (→ `tm-workflow`); checks git identity before main-branch access | `crates/trusty-agents-common/src/assets/agents/version-control.md`, `extends: base-ops` |
...
`documentation`, the one core agent whose work is genuinely mechanical
relative to the rest of the roster; every other core agent is sonnet.
`version-control` was haiku until the owner ruling of 2026-09-09 (#7274)
gave it a derived PR standard — component labels read off its own diff, and
a project and milestone read off the issue its `Refs #N` names — which is
judgment, not procedure. No core universal agent uses opus. Per DOC-61 §3.3, `model:` is set
