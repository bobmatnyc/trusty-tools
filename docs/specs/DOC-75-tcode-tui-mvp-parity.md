...
**Builds on:** [ADR-0063](../adr/0063-tui-is-the-primary-interactive-surface.md) — the TUI is trusty-code's primary interactive surface. [DOC-50](./DOC-50-tcode-tui-claude-code-clone.md) §4 ([`SPEC-TTUI-04~draft`](./DOC-50-tcode-tui-claude-code-clone.md#SPEC-TTUI-04~draft)) — the TUI's own phasing/MVP-scope section, which this document narrows to a milestone cut. Epic [#7939](https://github.com/bobmatnyc/trusty-tools/issues/7939) — the tracking issue this spec is the record for. [#2063](https://github.com/bobmatnyc/trusty-tools/issues/2063) — daemon hardening the interactive loop depends on.
...
| Persistent statusline | Present | Absent | [#8182](https://github.com/bobmatnyc/trusty-tools/issues/8182) (statusline only, in MVP) |
...
| 4 | [#8182](https://github.com/bobmatnyc/trusty-tools/issues/8182) — persistent statusline | M | reduced scope: statusline only, no subagent list |
...
PM delegation and subagent-list rendering, the `/model` picker,
degraded-terminal fallback, IDE/hooks/plugins/MCP authoring, cost/billing UI.
...
[ADR-0063](../adr/0063-tui-is-the-primary-interactive-surface.md) — TUI as
primary surface. [DOC-50](./DOC-50-tcode-tui-claude-code-clone.md) — the
TUI's own functional spec. Epic
