...
| DOC-62 | `SPEC-OKGSRC-01~draft` … `-10~draft` | [OKG Sources: Per-Assistant Knowledge Sources, Scheduled Refresh, and the Untrusted-Content Boundary](./DOC-62-okg-sources.md) | trusty-agents — assistant home / OKG store, source catalog, scheduled refresh, credential consumption, Knowledge config pane; trusty-kb — `okg` engine; trusty-search — index over the store |
...
> **Next free `DOC-N` = `DOC-63`** (updated 2026-08-01 — DOC-62 claimed by
> [OKG Sources](./DOC-62-okg-sources.md); DOC-60 claimed by
