...
| DOC-50 | `SPEC-TTUI-01~draft` … `-09~draft` | [trusty-code Interactive TUI: Claude Code Clone over Shared REPL Layer](./DOC-50-tcode-tui-claude-code-clone.md) | trusty-code — interactive terminal UI thin client; trusty-code-tui shared crate (ratatui REPL extraction from trusty-agents) |
...
| DOC-59 | `SPEC-PMINSTR-01~draft` … `-12~draft` | [Instruction Authority, Compilation, Workflow, and Consistency](./SPEC-PMINSTR-01-p1-p2-instruction-restructure.md) | trusty-mpm instruction pipeline and its cross-product consumers; includes current compilation/provenance (§11) and workflow/ticketing/version-control ownership (§12) — #4071, #5202, #5206 |
...
| DOC-71 | `SPEC-AUDITTPL-01~draft` … `-08~draft` | [Audit Dimensions and Templates](./DOC-71-audit-dimensions-and-templates.md) | `trusty-audit` — dimension query tables and the engagement manifest; `trusty-review` — report narrative templates and section instructions; `trusty-git-analytics` (tga) — agentic-commit detection and authorship data consumed by two of the new dimensions. Distills established technical-DD practice into a 16(+1)-dimension checklist, grounds a gap assessment against the current six-dimension evidence chain, and specifies a two-half template model (markdown narrative + Rust dimension-query table) that extends `DD_DIMENSIONS` without moving the query-intelligence-lives-in-trusty-audit ruling |
| DOC-72 | `SPEC-ANALYZELSP-01~draft` … `-11~draft` | [trusty-analyze On-Demand Language-Server Capability](./DOC-72-analyze-on-demand-language-server.md) | `trusty-analyze` — the language-server supervisor, the versioned result store, and the new `analyze.lsp_*` socket/MCP methods; `trusty-common` — the on-demand entry point (`uds::OnDemandAnalyze`, #6350); `trusty-mpm` — the new bundled `analyze` skill; `trusty-agents-common` — the `BASE-ENGINEER.md` tool-use instructions; `trusty-review` — the audit path's only client of trusty-analyze. Records the owner ruling of 2026-09-01 on #6589 (the Claude Code LSP plugin is not enabled by default; the capability moves into analyze and is used judiciously) and the scope expansion that makes the tools part of the analyze skill, the base coding agent's instructions, and the audit toolkit. The batch checkers stay the merge-chain gate; navigation stays on the `trusty-search` AST index |
...
| DOC-75 | `SPEC-TCMVP-01~draft` … `-06~draft` | [trusty-code v0.7.0 · Claude Code TUI Parity + PM-Delegated Coding Tasks](./DOC-75-tcode-tui-mvp-parity.md) | `trusty-code` — `tcode tui`, `trusty-code-tui` shared REPL crate, `tui_client::engine`, `task::executor` tool registry. Spec of record for milestone [87](https://github.com/bobmatnyc/trusty-tools/milestone/87); narrows epic [#7939](https://github.com/bobmatnyc/trusty-tools/issues/7939) to two tracks over [DOC-50](./DOC-50-tcode-tui-claude-code-clone.md) §4: the solo-agent-default cut, and (owner ruling 2026-09-19) a PM-delegated coding-task cut |
...
| DOC-77 | `SPEC-TELEM-01~draft` … `-16~draft` | [Runtime-Switchable Telemetry for Trusty Daemons](./DOC-77-runtime-switchable-telemetry.md) | new workspace crate `trusty-telemetry` (unpublished) — always-compiled lightweight core, MCP tool triplet (`telemetry_set`/`telemetry_status`/`telemetry_summary`) beside each daemon's `console_metrics`, on the existing UDS same-UID surface; first consumer `trusty-search`. Records the owner rulings of 2026-09-19 on epic [#8295](https://github.com/bobmatnyc/trusty-tools/issues/8295): a Cargo feature is never the on/off switch, telemetry never persists across a restart, and heavy tools (`console-subscriber`, `dhat`, `pprof-rs`, and a `samply` trial) stay feature-gated and rebuild-only. Phase 1 is [#8296](https://github.com/bobmatnyc/trusty-tools/issues/8296). §§11-16 (added 2026-09-19) specify phase 2's per-session/per-agent telemetry attribution across the harness daemons (`trusty-mpm`, `trusty-code`, `trusty-agents`), the console AGGREGATE / crate-dashboard DETAIL views, and the data contract [#8298](https://github.com/bobmatnyc/trusty-tools/issues/8298)'s load-based admission rule reads |
...
> outranks DOC-39, DOC-48, DOC-50, DOC-51, DOC-60, DOC-61, DOC-62, and DOC-65
> on whether trusty-code's behavior ships, is partial, deferred, or
> superseded (#5433). It complements those specs' behavioral detail; it does
> not replace it.
...
> named `DOC-72` as next free. **Next free `DOC-N` = `DOC-78`** (updated
> 2026-09-19 — `DOC-77` claimed by
> [Runtime-Switchable Telemetry for Trusty Daemons](./DOC-77-runtime-switchable-telemetry.md),
> verified free on `origin/main` per that document's own DOC-N claim line).
> Prior note, retained for history: 2026-09-16 — `DOC-76` claimed by
...
> [The Unified mpm/code/agents Dashboard](./DOC-73-unified-mpm-code-agents-dashboard.md),
> verified free on `origin/main` (`734b61e13`) and on
> `origin/docs/6606-analyze-lsp-spec`: no filename or self-label claim under
> `docs/specs/**` on either branch beyond this catalog note, no open pull request
> claiming `DOC-73`, `scripts/check_doc_numbers.sh` clean (132 docs / 126 claims,
> 3 grandfathered, 0 violations) before the file was added.
...
> **Next free `DOC-N` = `DOC-73`** (updated 2026-09-01 — DOC-72 claimed by
> [trusty-analyze On-Demand Language-Server Capability](./DOC-72-analyze-on-demand-language-server.md),
> verified free on `origin/main` (`22e0cc74d`): the only `DOC-72` mention
> anywhere in the tree was this catalog note itself, no open pull request claims
> a `DOC-N` (one PR open, unrelated), and `scripts/check_doc_numbers.sh` was
> clean (132 docs / 126 claims, 3 grandfathered, 0 violations) before the file
> was added.
>
> Previously — updated 2026-08-21 — DOC-71 claimed by
> [Audit Dimensions and Templates](./DOC-71-audit-dimensions-and-templates.md),
> verified free on `origin/main` (`973cce38`): no filename or self-label claim
> under `docs/specs/**` beyond this catalog note, no open pull request claiming
> `DOC-71`/`DOC-72`, `scripts/check_doc_numbers.sh` clean (0 violations) before
> the file was added.
>
> Previously — updated 2026-08-13 — DOC-70 claimed by
...
> added.
>
> Before that — updated 2026-08-12 — DOC-69 claimed by
...
> under `docs/specs/**`, no open pull request claiming a `DOC-N`,
> `check_doc_numbers.sh` clean (100 docs / 94 claims) before the file was added.
> Previously — updated 2026-08-04 — DOC-65 claimed by
