...
| DOC-66 | `SPEC-SESSWS-01~draft` … `-06~draft` | [Session and Workstream in trusty-mpm: the 1:N model, lifecycle, and slot semantics](./DOC-66-session-workstream-model.md) | trusty-mpm — session/workstream data model, launch and provisioning paths, worktree reclamation. Encodes [ADR-0030](../adr/0030-sessions-own-many-workstreams-from-the-main-checkout.md): one session owns many workstreams, the session's home is the main checkout, the ~5 workstream cap is an advisory attention limit. Re-scopes DOC-52 §1.5/§3.1 |
