...
| DOC-44 | `SPEC-TWIN-01~draft` … `-04~draft` | [Engineering Lead / Virtual Twin Cross-Tool Orchestration Architecture](./DOC-44-engineering-lead-twin-orchestration.md) | trusty-agents — multi-workstream lead orchestration / cross-tool supervision |

> **Catalog note — `DOC-42` & `DOC-43` occupied.** `DOC-42` is claimed by [`agent-bundled-skills.md`](./agent-bundled-skills.md) (shipped in tm 0.19.22); `DOC-43` is reserved for an in-flight requirements standard (PR #3019). This catalog skips to **`DOC-44`** for the engineering-lead spec. **Next free `DOC-N` = `DOC-45`** (scan-before-claim per [DOC-38 §4.1](./spec-linked-documentation.md)).
