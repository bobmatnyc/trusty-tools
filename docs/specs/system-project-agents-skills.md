...
Every spawn path — the routed local-checkout spawn (`spawn_managed_routed`, `lifecycle.rs:390`,
which [ADR-0055](../adr/0055-trusty-mpm-stops-creating-worktrees-the-sentinel-becomes-authoritative.md)
narrowed from the removed `spawn_managed_cloned`'s remote-clone fallback to a local-`repo_url`-only
route), in-project (`inproject.rs`), and the standalone driver (`standalone/load.rs:200`,
`run_prepare_session`) — converges on `prepare_session_inner` (`session_launch/mod.rs:238`). There
...
- [ADR-0055 — Trusty-mpm stops creating worktrees; the sentinel becomes authoritative](../adr/0055-trusty-mpm-stops-creating-worktrees-the-sentinel-becomes-authoritative.md) — removed `spawn_managed_cloned`'s remote-clone fallback; `session_new` now hard-errors on a non-local `repo_url` (`local_repo_url::require_local_repo_url`).
- `crates/trusty-mpm/src/daemon/managed_routes/lifecycle.rs` — `spawn_managed_routed` (390), local-checkout-only since ADR-0055. `inproject.rs` — base clone + `.worktrees/`; `DEFAULT_REPOS_DIR` (41).
- `crates/trusty-mpm/src/core/standalone/load.rs` — `run_prepare_session` (200) — the #1927 drift site; `load_alias` (65) is unaffected by ADR-0055 (direct `git` shell-outs, never routed through the removed provisioner).
