...
Twelve scripts in `scripts/` run in a GitHub Actions workflow and nowhere else —
...
`check_public_docs.sh` is listed as a thirteenth row for the opposite reason: it ran
...
| `ci-affected-test-plan.sh` | `bash scripts/ci-affected-test-plan.sh [--docs-only true\|false] [--max-legs N] -- <select-test-crates.sh args>` | `ci.yml` (`affected-plan`) | Plans `Rust tests (affected crates)`: runs `select-test-crates.sh`, returns nothing on a `docs_only=true` verdict, drops the four Tauri UI crates, and splits the rest into at most 8 legs, largest test suite first. Prints `count`, `crates`, `matrix` and `reason` as `key=value` lines, also appended to `$GITHUB_OUTPUT`. A failing selector fails the plan, so the roll-up goes red instead of reporting "nothing to test". See [ci-gates.md](ci-gates.md). |
