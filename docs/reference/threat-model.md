...
| **trusty-review** | `127.0.0.1:7891` | **No** — missed by the #3304/#3317 guard rollout; tracked **in progress**, issue [#3332](https://github.com/bobmatnyc/trusty-tools/issues/3332) | Yes (`review` → `trusty-review`) | No embedded UI | CLI, console proxy, HMAC-verified GitHub webhook (signature check is independent of the origin guard) |
...
| **trusty-mpm** | `127.0.0.1:7880`; optional `--tailscale` secondary listener (tailnet IP, same port) | **Yes on both listeners** — `guard_router` wraps the primary loopback listener and, when present, the Tailscale listener with the same `SelfOrigins` guard (`daemon/api/origin_guard.rs`) | Yes (`mpm` → `trusty-mpm`, #1849 Phase 1) | No embedded UI (separate `trusty-mpm-gui` Tauri app) | CLI (`tm`), native MCP stdio bridge, `trusty-mpm-gui` (talks **directly** to the daemon, not gateway-first — migration tracked as [#3333](https://github.com/bobmatnyc/trusty-tools/issues/3333), #1849 Phase 2), console proxy, and (while `--tailscale` remains) any tailnet peer directly — redundant with console's own Tailscale/Funnel modes and slated for removal ([#3330](https://github.com/bobmatnyc/trusty-tools/issues/3330), in progress) |
...
**Two items above are explicitly "in progress," not done:** trusty-agents'
bind/guard/allowlist fix (#3329, epic #3328) and trusty-mpm's `--tailscale`
listener removal (#3330). Do not read this table as claiming either is
resolved — it records the state on `origin/main` as of this writing.
