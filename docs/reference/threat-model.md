...
| **trusty-console** (event-bus ingest, #6848) | **No TCP listener.** One hardened Unix socket at `daemon_socket_path("trusty-console")` (`event_bus::ingest::ingest_socket_path`), bound through `bind_singleton_hardened` (#6848 fix round) so an unclean shutdown's leftover socket file is reclaimed rather than wedging every future bind | **n/a** — the trust boundary is the `0700` directory, the `0600` socket, and the `ensure_peer_is_self` uid check `handle_connection` runs on every accepted connection before a byte is read. No `SelfOrigins` guard: browser-CSRF machinery has no meaning on a socket | **n/a** — never proxied; this is console's own ingest path, not a route console forwards elsewhere | No | Every harness's `control_bus::PushClient` (`trusty-mpm`, `trusty-code`, `trusty-agents`, `trusty-analyze` — DOC-73 §4.1/§4.2), each holding a connection and streaming newline-delimited `HarnessEvent` JSON. Bounded against a hostile or broken peer on four axes: the `ensure_peer_is_self` uid check rejecting a connection before a byte is read, a 1 MiB per-line read cap applied BEFORE each read (`ingest::MAX_LINE_BYTES`, via a fresh `.take()` budget rather than checked only after an unbounded read returns), a 60 s per-connection idle timeout (`ingest::READ_IDLE_TIMEOUT`), and a 256-connection concurrency cap (`ingest::MAX_CONCURRENT_CONNECTIONS`, a `tokio::sync::Semaphore` acquired before a connection's handler is spawned) |
