...
| `FORCE_UI_BUILD` | `trusty-memory` `build.rs` (issue #5078) | Set to `1` to rebuild `ui/dist/` even when the committed bundle already matches the UI source, then re-stamp it via `scripts/stamp-ui-bundle.sh`. Without it a cargo build keeps the committed bundle, because the install and `vite build` steps rewrite files git tracks (`ui/package-lock.json`, `ui/dist/ui-source-hash.txt`). Reach for it after editing `crates/trusty-memory/ui/src/`. |
