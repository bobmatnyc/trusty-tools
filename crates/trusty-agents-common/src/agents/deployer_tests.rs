...
// ── #7688: the flag-gated composed-body suffix ──────────────────────────────

/// Two agents in a source tree, one extending the other, so the suffix is
/// asserted against a composed CHAIN rather than a single file.
fn suffix_fixture() -> (TempDir, TempDir) {
    let src = TempDir::new().unwrap();
    let tgt = TempDir::new().unwrap();
    fs::write(
        src.path().join("BASE-AGENT.md"),
        "---\nname: base-agent\nrole: engineer\n---\n\n# BASE\n\nBASE BODY\n",
    )
    .unwrap();
    fs::write(
...
/// 🔴 THE OFF CASE. Passing no suffix must produce byte-for-byte what the
/// pre-#7688 deployer produced — the pin requirement 3 asks for.
#[test]
fn deploy_without_a_suffix_is_byte_identical() {
    let (src, legacy_tgt) = suffix_fixture();
    let suffix_tgt = TempDir::new().unwrap();

    deploy_agents(src.path(), legacy_tgt.path()).unwrap();
    deploy_agents_filtered_with_suffix(src.path(), suffix_tgt.path(), |_| true, None).unwrap();

    for name in ["BASE-AGENT.md", "engineer.md"] {
        assert_eq!(
            fs::read_to_string(legacy_tgt.path().join(name)).unwrap(),
            fs::read_to_string(suffix_tgt.path().join(name)).unwrap(),
            "{name} must be byte-identical when no suffix is supplied"
        );
    }
}

/// THE ON CASE — every deployed agent carries the text, not just the base.
#[test]
fn deploy_appends_the_suffix_to_every_agent() {
    let (src, tgt) = suffix_fixture();
    deploy_agents_filtered_with_suffix(src.path(), tgt.path(), |_| true, Some("\n\nADDENDUM\n"))
        .unwrap();

    for name in ["BASE-AGENT.md", "engineer.md"] {
        let deployed = fs::read_to_string(tgt.path().join(name)).unwrap();
        assert!(
            deployed.ends_with("ADDENDUM\n"),
            "{name} must end with the suffix, got {deployed:?}"
        );
    }
}

/// The suffix lands in the BODY; the frontmatter the composer rendered is
/// untouched, so a strict YAML consumer still parses the file.
#[test]
fn the_suffix_does_not_disturb_the_frontmatter() {
    let (src, tgt) = suffix_fixture();
    deploy_agents_filtered_with_suffix(src.path(), tgt.path(), |_| true, Some("\n\nADDENDUM\n"))
        .unwrap();

    let deployed = fs::read_to_string(tgt.path().join("engineer.md")).unwrap();
    assert!(deployed.starts_with("---\n"));
    assert!(deployed.contains("provenance: framework-owned"));
    // The suffix sits after the body, so the SECOND `---` still closes the
    // frontmatter and nothing in the suffix can be read as a key.
    let body_start = deployed[4..].find("---\n").expect("frontmatter closes") + 4;
    assert!(deployed[body_start..].contains("ADDENDUM"));
}

/// 🔴 The ledger must record the bytes that landed, or the next deploy sees a
/// checksum mismatch and treats a file it wrote as user-modified — the #4698
/// invariant, restated for the suffix.
#[test]
fn the_manifest_records_the_suffixed_checksum() {
    let (src, tgt) = suffix_fixture();
    deploy_agents_filtered_with_suffix(src.path(), tgt.path(), |_| true, Some("\n\nADDENDUM\n"))
        .unwrap();

    let on_disk = fs::read_to_string(tgt.path().join("engineer.md")).unwrap();
    assert!(
        AgentManifest::load(tgt.path()).checksum_matches("engineer.md", &on_disk),
        "the ledger must describe the suffixed bytes"
    );

    // And a second identical deploy is therefore a no-op, not a rewrite.
    let again = deploy_agents_filtered_with_suffix(
        src.path(),
        tgt.path(),
        |_| true,
        Some("\n\nADDENDUM\n"),
    ...
    assert!(
        ...
        "an unchanged suffixed deploy must write nothing, got {:?}",
        again.deployed
    );
}

/// Flipping the flag refreshes the managed file rather than stranding it.
#[test]
fn removing_the_suffix_refreshes_the_deployed_file() {
    let (src, tgt) = suffix_fixture();
    deploy_agents_filtered_with_suffix(src.path(), tgt.path(), |_| true, Some("\n\nADDENDUM\n"))
        .unwrap();
    deploy_agents_filtered_with_suffix(src.path(), tgt.path(), |_| true, None).unwrap();

    let deployed = fs::read_to_string(tgt.path().join("engineer.md")).unwrap();
    assert!(
        !deployed.contains("ADDENDUM"),
        "turning the flag off must strip the addendum on the next deploy"
    );
}
