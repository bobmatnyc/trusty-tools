...
//! What: five submodules — [`frontmatter`] (the shared `key: value` line
//! parser, plus the `skills:` list-value grammar), [`builder`] (the
//! `extends:` inheritance-chain composer), [`manifest`] (the checksum +
//! atomic-write ownership ledger), [`deployer`] (writes composed agents into
//! a target directory, consulting the manifest to avoid clobbering user
//! edits, and records each processed agent's declared `skills:` for
//! co-deployment), and [`metadata`] (a read-only projection of a deployed
//! agent's frontmatter for display/diagnostics, used by `deployer` and by
//! trusty-mpm's `tm doctor` / `tm agent` surfaces). `trusty-mpm` re-exports
//! every public item from `crate::core::{agent_builder,agent_manifest,
//! agent_deployer,agent_metadata,frontmatter}` for source compatibility with
