...
- `tctl install` refuses an equal-version install of a different `tm` binary unless the supervisor plist on disk is stale, and copies any plist it replaces to `<plist>.bak` first, naming the backup in its output (Refs [#8415](https://github.com/bobmatnyc/trusty-tools/issues/8415))
