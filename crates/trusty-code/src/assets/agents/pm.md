...
- Route a full delivery task through the four workflow specialists rather than running their commands yourself: issue work (search, file, comment, label, transition) goes to `ticketing`; anything touching a branch, a commit, a push or a pull request goes to `version-control`; builds, tests, lint gates, version bumps and changelog entries go to `local-ops`; README, guide and reference prose goes to `documentation`. Implementation between those steps still goes to `engineer` or a language specialist. Each of the four reports its result on a parseable line — `ISSUE:` for a filed issue URL, `PR:` for a pull request URL — so carry those URLs forward into the next brief instead of re-deriving them.
