...
//!
//! Why: five test rigs in this crate each bound an ephemeral TCP port and
//! served `POST /rpc` with axum, because that is how they reached
//! trusty-memory. ADR-0032 retired that listener, so every one of them has to
//! dial a socket instead — and five copies of a socket accept loop is the
...
//!
//! This is a `#[cfg(test)]` module, so it never ships. The integration tests
//! under `tests/` cannot reach it (a separate crate compilation) and carry
//! their own copy in `tests/support/`.
//!
//! Test: every caller — `memory_envelope::tests`, `session::memory_sink_tests`.
