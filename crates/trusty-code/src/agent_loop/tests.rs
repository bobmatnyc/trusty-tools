...
//
// Why: log-content assertions in this module (and its `cadence`/
// `gate_intercept`/`redundant_rerun` children) go through
// `crate::test_support::begin_capture`/`captured_at_least` — see that
// module's doc for why the earlier per-test `tracing::subscriber::set_default`
// approach flaked under a wide `cargo test` thread pool and what replaced it.
...
// #2346 cadence-compressor turn-boundary wiring + #2349 threshold-compaction
// regression-signal tests live in a focused child module to keep this file
// under its SLOC cap.
mod cadence;
