...
//!
//! Why: the rigs in this crate each bound an ephemeral TCP port and served
//! `POST /rpc` with axum, because that is how they reached trusty-memory.
//! ADR-0032 retired that listener, so every one of them has to dial a socket
//! instead — and a copy of the accept loop per rig is the duplication the
//! workspace's common-entry-point rule exists to prevent.
//!
...
//! real daemon uses, and serves until the returned [`MockMemoryDaemon`] drops.
...
//!
//! This is the integration-test copy. `trusty_code::uds_mock` is the same
//! module for the crate's own unit tests; an integration test compiles as a
...
pub struct MockMemoryDaemon {
    socket: PathBuf,
    _dir: TempDir,
    shutdown: Option<tokio::sync::oneshot::Sender<()>>,
}

impl MockMemoryDaemon {
...
impl Drop for MockMemoryDaemon {
    fn drop(&mut self) {
...
pub async fn spawn<F>(handler: F) -> MockMemoryDaemon
where
    F: Fn(&str, Value) -> MockFuture + Send + Sync + 'static,
{
