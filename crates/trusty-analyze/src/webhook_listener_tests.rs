//! Coverage for `trusty-analyze`'s half of the webhook UDS listener (#5182).
//!
...
fn socket_path_matches_the_shared_contract() {
    ...
    assert_eq!(
        ...
        trusty_common::uds::scratch_socket_dir()
            .join(trusty_common::webhook_relay::ANALYZE_SOCKET_FILE)
    );
    assert_eq!(
        socket_path(),
        trusty_common::webhook_relay::socket_path_for(trusty_common::webhook_relay::ANALYZE_SOURCE)
            .expect("analyze is a configured source"),
...
fn inbox_root_matches_the_shared_contract() {
    ...
    assert_eq!(
        ...
        trusty_common::webhook_relay::inbox_root_for(trusty_common::webhook_relay::ANALYZE_SOURCE)
            .expect("analyze is a configured source")
...
fn inbox_root_lives_under_the_review_data_dir() {
    ...
    assert!(
        root.parent().is_some_and(|p| p.ends_with("trusty-analyze")),
