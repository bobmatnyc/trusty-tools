//! Handler for `trusty-analyze socket` — report the daemon's endpoint.
//!
//! Why: operators and scripts need to discover where the running daemon answers
//! without guessing. This is the migration of `trusty-analyze port` (#956),
//! which read the `http_addr` discovery file and printed a TCP port. #6287
//! (ADR-0032) retired both: there is no port, and there is no discovery file —
//! the socket path is DERIVED from the data directory, so the daemon and its
//! consumers cannot disagree about it and there is nothing for a stale file to
//! contradict.
//!
...
//! nothing is — preserving the property that `$(trusty-analyze socket)` fails
//! cleanly rather than handing a caller a path to a dead socket.
//!
//! What: prints one of two formats to stdout:
//!   - default:  the bare path  →  `/Users/x/…/trusty-analyze.sock\n`
...
/// Entry point for `trusty-analyze socket [--json]`.
///
...
/// `trusty_common::uds::socket_is_serving` — a bare connect — rather than an
/// `analyze.health` call, because the question here is "is the endpoint live",
/// and a daemon that is up but degraded (trusty-search down) must not be
/// reported as absent.
///
...
/// ONLY when the socket is live, so `$(trusty-analyze socket)` yields an empty
...
pub async fn handle_socket(format: SocketFormat) -> Result<()> {
    ...
    if !serving {
        anyhow::bail!(
            "trusty-analyze: nothing is serving {} — start it with \
...
mod tests {
    ...
    /// other than the bare path breaks `nc -U $(trusty-analyze socket)`.
    /// Test: this is the test.
    #[test]
    fn format_output_path() {
        let p = PathBuf::from("/tmp/trusty-analyze.sock");
        assert_eq!(
            format_output(&p, true, SocketFormat::Path),
            "/tmp/trusty-analyze.sock"
        );
    }
    ...
    fn format_output_json() {
        ...
        assert_eq!(
            ...
            r#"{"serving":true,"socket":"/tmp/trusty-analyze.sock"}"#
        );
        assert_eq!(
            format_output(&p, false, SocketFormat::Json),
            r#"{"serving":false,"socket":"/tmp/trusty-analyze.sock"}"#
        );
    }
