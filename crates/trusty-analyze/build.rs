...
fn main() {
    ...
    // #5078: `cargo test -p trusty-analyze` used to run the UI install and
    ...
    // fallback install rewrote `ui/package-lock.json` with the host platform's
    ...
    if std::env::var("FORCE_UI_BUILD").as_deref() != Ok("1")
        && committed_bundle_is_fresh(&crate_root, &dist_dir, "trusty-analyze")
    {
        return;
    }

    if build_svelte_ui(&ui_dir, &dist_dir, "trusty-analyze") {
        restamp_bundle(&crate_root, "trusty-analyze");
    }
}
