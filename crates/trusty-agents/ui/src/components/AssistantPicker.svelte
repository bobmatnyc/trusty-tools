<script lang="ts">
  /** Assistant instances and creation, excluding the internal Concierge helper.
   * Selection persists through activeAgentId; creation refreshes the roster.
   ...
   * What: best-effort refresh; assistant creation remains available while loading.
   */
  onMount(() => {
   ...
   * that does not yet contain the new id and clear the explicit selection.
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <span class="min-w-0 flex-1">
            ...
            >
              {card.origin === 'overlay' ? 'Your assistant' : 'Assistant'}
            </span>
          </span>
        </div>
        {#if card.description}
