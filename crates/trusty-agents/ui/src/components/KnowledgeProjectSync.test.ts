...
vi.mock('../stores/workspace', () => ({ assistantKnowledgeAttachments: writable([{ chat_id: 'local-chat', projects: ['/attached'] }]) }));
vi.mock('../lib/assistantKnowledge', () => ({ syncKnowledgeProjects: vi.fn() }));
...
afterEach(async () => { if (component) await unmount(component); component = undefined; document.body.innerHTML = ''; vi.resetAllMocks(); activeAgentId.set('izzie'); });
