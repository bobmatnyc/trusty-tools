...
//! trusty-memory's REST routes with axum. ADR-0032 retired that listener, so
//! every one of them has to dial a socket instead — and a copy of the accept
//! loop per rig is the duplication the workspace's common-entry-point rule
//! exists to prevent.
//!
...
//!
//! Test: every caller — `memory::trusty_client::tests`,
//! `api::server::workstreams::tests`.
