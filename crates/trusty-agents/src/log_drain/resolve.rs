...
//! scoped to this daemon's single-project shape — see the module docs on
//! `super` for why this differs from trusty-mpm's `sources[]` grouping.
//! What: [`LogDrainConfig`] is the on-disk shape, using the same field names
//! as trusty-mpm's `log_drain:` section; [`resolve_log_drain`] validates it
//! whenever present (even while disabled — a destination typo must surface
//! before the operator flips `enabled = true`), then resolves identity ONLY
//! for an enabled plan, since that step reads git.
//! Test: `tests`.
...
///
/// Why: modelled here, not parsed independently — `GlobalConfig::save()`
/// re-serializes only the fields declared on that struct, so an unmodelled
/// `[log_drain]` table would be silently dropped by any unrelated
/// `mcp_*`-tool write.
...
pub struct LogDrainConfig {
    ...
    /// Repository owner, when this daemon has no bound project (or its root
    /// has no git origin).
    #[serde(default)]
...
pub enum LogDrainConfigError {
    ...
    #[error(
        "log_drain has no owner/project: {reason} — set `owner`/`project` in \
         ~/.trusty-agents/config.toml's [log_drain] section, or run this daemon \
...
pub struct ResolvedLogDrain {
    ...
    /// This daemon's own log directory — the only source it drains.
