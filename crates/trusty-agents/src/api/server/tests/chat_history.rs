...
//!
//! What: newest-slice bounding; the `before` cursor paging backwards without a
//! gap or overlap; `has_more`; no palace bound / session absent / daemon
//! undiscoverable / daemon unreachable → `200` + `available: false`; unknown
//! agent → 404; traversal name → 400; and the route being wired into the real
//! router.
...
/// `before` walks backwards a page at a time. Two consecutive pages must abut
/// exactly: no message skipped, none served twice.
#[tokio::test]
async fn chat_history_before_cursor_pages_backwards() {
    let dir = agent_dir("izzie", BOUND_FIXTURE);
    let daemon = mock_memory(10).await;
    let dirs = [dir.path().to_path_buf()];

    let first = body_json(chat_history_at(&dirs, "izzie", Some(daemon.socket()), 4, 0).await).await;
    let second =
        body_json(chat_history_at(&dirs, "izzie", Some(daemon.socket()), 4, 4).await).await;
    ...
    let third = body_json(chat_history_at(&dirs, "izzie", Some(daemon.socket()), 4, 8).await).await;
...
async fn chat_history_degrades_when_memory_undiscoverable() {
    ...
    let body =
        body_json(chat_history_at(&[dir.path().to_path_buf()], "izzie", None, 100, 0).await).await;
    assert_eq!(body["available"], false);
    assert_eq!(body["palace"], "owner-profile");
    assert!(
        body["reason"]
...
async fn chat_history_degrades_when_memory_unreachable() {
    ...
    let resp = chat_history_at(&[dir.path().to_path_buf()], "izzie", Some(dead), 100, 0).await;
    assert_eq!(
        resp.status(),
        StatusCode::OK,
...
async fn chat_history_unknown_agent_404() {
    let dir = agent_dir("izzie", BOUND_FIXTURE);
    let resp = chat_history_at(&[dir.path().to_path_buf()], "nobody", None, 100, 0).await;
...
async fn chat_history_rejects_traversal_name() {
    ...
    for name in ["..", ".", "../../etc/passwd", ""] {
        let resp = chat_history_at(&[dir.path().to_path_buf()], name, None, 100, 0).await;
        assert!(
