...
- **The `ompm` binary target** (issue #4046): a second, thin HTTP-client
  binary that shipped alongside `tagent` on the (factually incorrect) premise
  that it preserved backward compat for scripts using an old `ompm` binary
  name — git history shows `ompm` and `open-mpm` existed as two separate
  binaries from the monorepo's first commit, and `open-mpm` (not `ompm`) was
  the one later renamed to `tagent`. Nothing has ever depended on `ompm`: zero
  invocations across the repo, CI, runbooks, shell profiles, or LaunchAgents.
  trusty-agents has never published to crates.io, so removing it now avoids
  locking in an unused binary at the first publish. `tagent` is unaffected —
  it does not consume `src/bin/ompm.rs`.
