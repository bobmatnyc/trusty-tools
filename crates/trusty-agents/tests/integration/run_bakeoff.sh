...
# What: Invokes `./tagent --workflow prescriptive --task-file task.txt` and
...
if [[ ! -x "$TEST_DIR/tagent" ]]; then
  echo "❌ binary missing: $TEST_DIR/tagent" >&2
  exit 1
fi

echo "=== Running bake-off Level $LEVEL in $TEST_DIR ==="
cd "$TEST_DIR"
./tagent --workflow prescriptive --task-file task.txt
