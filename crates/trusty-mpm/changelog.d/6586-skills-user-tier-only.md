...
- Bundled `tm-*` skills are deployed to the user tier only. The three sites that
  ...
  (#6586).
- Session launch and `tm sessions sync-assets` deploy the bundled skill roster
  into the managed config dir's `skills/` instead — the direct counterpart of the
  bundled-agent destination #4409 gave them, reachable through the new
  `FrameworkPaths::managed_skills_dir`. `tm doctor`'s `deployment` completeness
  probe follows the roster to that tier, so probe and repair agree on where a
  workspace is complete; a leftover project-tier copy no longer counts toward it
  (#6586).
