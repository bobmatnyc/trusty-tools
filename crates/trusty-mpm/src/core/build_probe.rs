...
//!
//! **Sampling failure denies, it never counts zero.** A cap derived from a
//! process table nobody could read is not a cap, and admitting on a default of
//! zero is precisely the overcommit this module exists to stop. Every failure
//! arm returns [`BuildProbeError`], which the daemon reports and the guard
//! denies on — the same fail-closed direction `pm_guard_builder_cap` already
//! takes for an unreachable daemon.
//!
...
/// spawning a compiler. A test hands it a constructed tree; the daemon hands it
...
/// Test: `a_fake_sampler_drives_the_whole_tally`.
...
/// call. One long-lived instance in the daemon gives every claim a meaningful
/// number; constructing one per claim would report every build as idle.
...
/// daemon attributes a group to the session that started it.
/// Test: `a_cargo_build_collapses_to_one_group`,
...
mod tests {
    ...
    fn ordinary_processes_are_not_compilers() {
        for name in ["python3", "node", "tsc", "bash", "claude", "tm", "terraform"] {
            assert!(
    ...
    /// The group carries the pid chain the daemon attributes sessions by.
    #[test]
