...
//! is already flattened (see [`crate::core::agent_builder::compose_agent`]'s
//! `skills:` union-and-emit), and a source file with no `extends:` parses
//! identically either way. Duplicating [`crate::core::agent_builder`]'s
//! internal frontmatter grammar here would drift from the parser used at
//! compose/deploy time, so this module reuses
//! [`crate::core::agent_builder::split_frontmatter`] (made `pub(crate)` for
//! exactly this purpose) and projects its private [`Frontmatter`] into a
//! public, stable-shaped struct external callers may read.
...
use super::agent_builder::{Frontmatter, split_frontmatter};
