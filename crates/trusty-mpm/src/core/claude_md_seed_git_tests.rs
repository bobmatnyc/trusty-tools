...
/// A repository three levels down sits outside the bounded scan — cheap by
/// design, not exhaustive.
#[test]
fn a_repository_three_levels_down_is_not_found() {
    let tmp = TempDir::new().unwrap();
    let dir = std::fs::canonicalize(tmp.path()).unwrap().join("workspace");
    let deep = dir.join("a").join("b").join("service");
