...
fn no_ancestors_yields_nothing() {
    ...
    assert!(scan_with_excludes(&project, &none()).unwrap().is_empty());
}
...
fn a_seed_ancestor_is_reported_as_a_seed() {
    ...
    let found = scan_with_excludes(&project, &none()).unwrap();
...
fn an_excluded_ancestor_is_flagged_excluded() {
    ...
    let found = scan_with_excludes(&project, &excludes).unwrap();
...
fn scan_does_not_report_the_git_roots_own_claude_md_for_a_nested_project_root() {
    ...
    let found = scan(&nested, None, None).unwrap();
...
fn scan_reads_the_excludes_from_the_project_layer() {
    ...
    let found = scan(&project, None, None).unwrap();
...
fn a_marker_project_inside_two_repos_reports_both_repo_roots() {
    ...
    let found = scan(&project.join("sub"), None, None).unwrap();
...
fn a_symlinked_start_resolves_like_its_target() {
    ...
    let found = scan(&link, None, None).unwrap();
    assert_eq!(
...
/// FAILS AGAINST ROUND 3: an ancestor the scan cannot stat was skipped, so the
/// scan came back empty. It is an error. Skipped when running as root, where
/// permission bits do not bind.
#[cfg(unix)]
#[test]
fn an_unreadable_ancestor_is_an_error_not_an_empty_scan() {
    use std::os::unix::fs::PermissionsExt;
    let (_tmp, ancestor, project) = fixture();
    std::fs::write(ancestor.join("CLAUDE.md"), "# notes\n").unwrap();
    std::fs::set_permissions(&ancestor, std::fs::Permissions::from_mode(0o000)).unwrap();
    let readable_anyway = std::fs::metadata(ancestor.join("CLAUDE.md")).is_ok();

    let resolved = resolve_project_root(&project, None);
...
fn the_home_directory_is_never_a_project_boundary() {
    ...
    let found = scan(&scratch, Some(&home), None).unwrap();
    assert_eq!(
        found.iter().map(|f| &f.path).collect::<Vec<_>>(),
        vec![&file],
        "{found:?}"
...
fn a_failed_scan_is_announced_not_silent() {
    ...
    assert_eq!(scan_notice(Path::new("/x"), &Ok(Vec::new())), None);
}
