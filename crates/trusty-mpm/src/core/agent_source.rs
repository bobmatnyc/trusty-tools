...
fn deploy_into(
    ...
    target_dir: &Path,
    skills_root: &Path,
    out: &mut AgentAutodeploy,
) {
    match deploy_agents(source_dir, target_dir, skills_root) {
        Ok(result) => {
...
pub fn autodeploy_agents_for(
    ...
    autodeploy_agents(&paths.agents, target_dir, skills_root)
}
