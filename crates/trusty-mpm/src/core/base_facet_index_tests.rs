...
//! #4255); the one that asserts on the wire body stands up its own loopback
//! listener and points discovery at it.
//!
...
///
/// Why: `base_facet_posts_the_base_checkout_with_the_vector_lane_on` has to
/// redirect daemon discovery and opt out of the #4255 write suppression, and a
/// panic between set and restore would leak process-global state into sibling
/// serial tests.
...
///
/// Why: every other test here observes the outcome value, which a well-meaning
/// refactor could keep correct while the request on the wire changed. This one
/// asserts on the bytes the daemon would actually receive — the same technique
/// `session_launch::tests_search_index` uses for the `allow_sensitive_path`
/// invariant — so both halves of the fix (which root, which lane) are pinned
/// where they are observable.
/// What: points `TRUSTY_DATA_DIR_OVERRIDE` at an isolated data dir, writes a
/// fake daemon's bound address to `<data_dir>/trusty-search/http_addr`, opts out
/// of the #4255 write suppression (safe: discovery now points at this test's own
/// loopback socket), and inspects the captured body. `#[serial]` because both
...
fn base_facet_posts_the_base_checkout_with_the_vector_lane_on() {
    ...
    // Fake daemon: accept one connection, capture the body, answer 200, then
    // close so the follow-up reindex call fails fast instead of idling in the
    ...
    let body: serde_json::Value = serde_json::from_str(
        &rx.recv_timeout(std::time::Duration::from_secs(5))
            .expect("the fake daemon must have received a create-index POST"),
    )
    ...
    assert_eq!(
        body.get("root_path").and_then(serde_json::Value::as_str),
        Some(expected_root.to_string_lossy().as_ref()),
        "the create-index POST must name the BASE checkout, not the worktree; got {body:?}"
    );
    assert_eq!(
