...
/// `cli_parses_mcp_remove_list_get`, `share_is_a_no_op_under_the_claude_code_standard`.
pub(crate) async fn dispatch(cmd: crate::cli::McpCmd) -> Result<()> {
    ...
    match cmd {
        ...
        // #7892: retired to a notice, not removed — scripts and docs still
        // spell these verbs.
        McpCmd::Share { name, .. } => share_cmd(&name, true),
        McpCmd::Unshare { name, .. } => share_cmd(&name, false),
...
/// What: `project` writes the project's own `.mcp.json` instead of the user
/// scope; `share_with_projects` is retired and only prints a notice (#7892).
/// Test: `cli_parses_mcp_share`, `cli_parses_mcp_unshare`.
#[derive(Debug, Clone, Copy, Default)]
pub(crate) struct AddPlacement {
    /// Write the server into THIS project's `.mcp.json` (`--project`).
    pub project: bool,
    /// `--share-with-projects`, retired by #7892: a user-scope server already
    /// loads in every project, so there is nothing left to share.
...
pub(crate) fn add_cmd(
    ...
    // `--project` writes the project's own `.mcp.json` — one declaration point,
    // per ADR-0042. // #7892: no trust gate. Whether a session CONNECTS that
    // entry is Claude Code's own approval decision, exactly as it is for a
    // `claude mcp add --scope project` run by hand.
    if project {
        ...
        if changed {
            ...
            println!(
                "  Claude Code approves project-scope servers itself — it prompts on the \n  \
                 next interactive session, or reads `enableAllProjectMcpServers` / \n  \
                 `enabledMcpjsonServers` from settings. `tm mcp list` shows that state."
            );
        } else {
            println!(
    ...
    if changed {
        ...
        println!(
            "  This is USER scope: since #7892 it loads in every tm session, in every \n  \
             project, with no grant."
        );
    } else {
    ...
    if share_with_projects {
        println!(
            "  --share-with-projects is retired (#7892): a user-scope server already \n  \
             loads in every project, so there is nothing left to share."
        );
    }
    Ok(())
...
/// Handle `tm mcp share <name>` and `tm mcp unshare <name>` — retired (#7892).
///
/// Why: sharing existed because #7422 made a user-scope server invisible to an
/// untrusted project unless the operator lent it out by content. Under the
/// Claude Code standard every user-scope server already loads in every project,
/// so a grant would grant nothing. The verbs stay so scripts and runbooks that
/// spell them keep exiting zero; they print what changed instead of writing a
/// store this crate no longer has.
/// What: one notice line, then `Ok(())`. `--root` is accepted and ignored —
/// there is no per-root state left to address.
///
/// # Errors
///
/// None; the signature stays `Result` so the dispatch arm is unchanged.
/// Test: `share_is_a_no_op_under_the_claude_code_standard`, `cli_parses_mcp_share`,
/// `cli_parses_mcp_unshare`.
pub(crate) fn share_cmd(name: &str, share: bool) -> Result<()> {
    let verb = if share { "share" } else { "unshare" };
    println!(
        "`tm mcp {verb} {name}` does nothing: since #7892 tm follows the Claude Code \n           standard. A user-scope server in the protected config loads in every session, \n           in every project, with no grant. A project's own .mcp.json follows Claude \n           Code's approval (`enableAllProjectMcpServers` / `enabledMcpjsonServers`)."
    );
    Ok(())
}
...
///
/// Why: drop a server the operator no longer wants in every managed session.
/// What: calls [`mcp_config::remove_server`] and says whether anything went.
/// // #7892: no share grant to settle — that store is gone.
...
///
/// Why: an operator needs to know what a session in this directory will
/// actually connect. Since #7892 that is an additive answer with three sources,
/// so the command names all three rather than marking rows `scoped-out`.
/// What: the user-scope `mcpServers` map as a table (name → type + target), the
/// trusty-* builtins tm adds through `--mcp-config`, and each `.mcp.json` entry
/// with the approval state
/// [`trusty_mpm::core::project_mcp_approval::project_mcp_state`] reads out of
/// Claude Code's own settings. `--json` prints the raw user-scope map alone,
/// unchanged, because that is what a script consumes.
...
pub(crate) fn list_cmd(root: Option<&str>, json: bool) -> Result<()> {
    ...
    if servers.is_empty() {
        ...
        println!(
            "User-scope MCP servers in {} ({}) — these load in EVERY tm session:",
            config_dir.display(),
            servers.len()
        );
    ...
    // #7892: the composed file tm passes to `--mcp-config`, additive on top of
    // the user scope above — and the one read that can degrade.
    let scope = trusty_mpm::core::session_mcp_scope::resolve_scope(&config_dir);
    println!();
    println!(
        "  tm adds {} builtin server(s) on top: {}",
        scope.included.len(),
        scope.included.join(", ")
    );
    ...
    let project = trusty_mpm::core::project_mcp_approval::project_mcp_state(&cwd, &config_dir);
    if !project.is_empty() {
        println!();
        println!(
            "  {}/{} declares {} server(s); Claude Code approves these itself:",
            cwd.display(),
            mcp_config::MCP_JSON,
            project.len()
        );
        for server in &project {
            println!("    {:<24}  {}", server.name, server.approval.label());
        }
        println!(
            "  Approve with `enableAllProjectMcpServers` or `enabledMcpjsonServers` in \n               settings, or answer the prompt on the next interactive session."
        );
    }
    Ok(())
}
...
pub(crate) fn get_cmd(root: Option<&str>, name: &str, json: bool) -> Result<()> {
    ...
    if json {
        ...
        // #7892: standard Claude Code user-scope semantics — every tm session
        // reading this config dir connects it, with no per-project grant.
        println!("          Loads in every tm session, in every project.");
    }
    Ok(())
}
...
mod tests {
    ...
    /// #7892: the retired verbs stay callable and say why they do nothing, so a
    /// runbook or script that still spells them keeps exiting zero.
    #[test]
    fn share_is_a_no_op_under_the_claude_code_standard() {
        share_cmd("apex", true).expect("share must not fail");
        share_cmd("apex", false).expect("unshare must not fail");
    }

    /// #7892, inverted from the `--project` trust gate: writing a project-scope
    /// server no longer needs a `tm project trust` grant, because Claude Code's
    /// own approval decides whether the session connects it.
    #[test]
    fn add_project_needs_no_trust_grant() {
        let tmp = tempfile::TempDir::new().unwrap();
        let cwd = tmp.path().join("repo");
        std::fs::create_dir_all(&cwd).unwrap();

        // `add_project_server` is the write `add_cmd --project` performs; call
        // it directly, because `add_cmd` resolves the process-wide cwd and this
        // binary's `env_isolation_tests` ratchet bans changing it.
        let changed = mcp_config::add_project_server(
            &cwd,
            "apex",
            serde_json::json!({"type": "http", "url": "https://example.invalid/mcp"}),
        )
        .expect("an untrusted project must still take the write");

        assert!(changed);
        let state = trusty_mpm::core::project_mcp_approval::project_mcp_state(&cwd, tmp.path());
        assert_eq!(state.len(), 1);
        assert_eq!(
            state[0].approval,
            trusty_mpm::core::project_mcp_approval::Approval::Prompt,
            "with no settings tier naming it, Claude Code decides, not tm"
        );
    }
}
