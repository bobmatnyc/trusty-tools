...
pub(crate) fn compose_session_instructions_with_roster(
    ...
    let mut stdin_prompt = stdin_git_init_prompt();
...
/// the reason on stderr, reads one line, and classifies it through the SAME
/// confirm helper the picker's other yes/no prompts use
/// ([`crate::commands::picker_delete::confirm_is_yes`]) rather than a new
/// stdin reader.
...
fn stdin_git_init_prompt() -> Option<Box<dyn FnMut() -> bool>> {
    ...
    Some(Box::new(|| {
        ...
        eprint!("not a git repository; initialise one? [y/N] ");
