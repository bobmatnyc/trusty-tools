...
pub(crate) const DOCTOR_CHECKS: &[(&str, &str)] = &[
    ...
    (
        "auto_memory",
        "Whether this project's auto-memory FALLBACK posture is correct (issue #7685). `trusty-memory` is the memory; Claude Code's own auto memory (`MEMORY.md`) stays on only while trusty-memory is unavailable (owner ruling 2026-09-12), and a managed launch turns it off two ways — the project-tier `autoMemoryEnabled: false` settings key and `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` on the spawned session — both written only when trusty-memory answered. Neither is observable afterwards, so this row reads the EFFECTIVE `autoMemoryEnabled` value across the same four layers Claude Code consults — project local, project, user local, user — names the layer it resolved from, reads whether the project's `MEMORY.md` still holds facts, and grades the pair against a live trusty-memory health probe. `Fail` when trusty-memory is healthy and auto memory is on (an unset key counts as on: Claude Code documents it ON by default), when a healthy trusty-memory sits beside an index that still holds facts, or when a settings file could not be parsed. `Warn` when trusty-memory is down and auto memory is carrying the project — the fallback working as designed. `Ok` when auto memory is off and the index is empty or absent. Read-only; `tm doctor --fix --yes` writes `false` into the PROJECT tier through the same merge the launch path uses, preserving every other key, and never rewrites a `settings.local.json` the operator owns. `--fix` never migrates — `tm memory import-auto-memory` stores each fact in the palace, archives the file, and only then empties the index, and the row names it.",
    ),
];
