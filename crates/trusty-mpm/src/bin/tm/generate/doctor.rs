...
pub(crate) const DOCTOR_CHECKS: &[(&str, &str)] = &[
    ...
    (
        "ancestor_claude_md",
        "Every `CLAUDE.md`, `CLAUDE.local.md` and `.claude/CLAUDE.md` found in a directory ABOVE the project root (issue #7673). Claude Code loads memory files from the session's cwd up to the filesystem root, so one file at `$HOME` is prepended to every turn of every agent in every project beneath it — and no other doctor row looks above the project at all, which is how a tm seed template sat at `$HOME` on 2026-09-12 costing every turn. Reports each file's path, byte size and a token estimate (bytes/4, a rule of thumb, not a measurement). `Ok` when nothing above the project loads — including when every such file is already named in `claudeMdExcludes`, which is read as the union across the project, user and managed settings layers exactly as a session merges it. `Warn` when a file with real content loads, since it may well be wanted. `Fail` when one is tm's own seed template with no project content added, which is pure cost. Read-only. `tm doctor --fix --yes` applies the two remedies the shape calls for: a pure seed template is RENAMED aside to `<name>.stale-seed-<YYYYMMDD>` (reversible, and refused when that name already exists), and a content-carrying file is left byte-for-byte alone with its absolute path added to `claudeMdExcludes` in the project's own `.claude/settings.local.json`. Nothing inside the project root is ever a finding or a target.",
    ),
];
