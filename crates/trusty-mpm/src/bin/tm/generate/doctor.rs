...
pub(crate) const DOCTOR_CHECKS: &[(&str, &str)] = &[
    ...
    (
        "session_scope",
        "Names what a project's sessions load — tm's trusty-* builtins, the operator's user-scope servers from the protected `.claude.json` (which load in every session since issue #7892), and each `.mcp.json` entry with the approval state Claude Code itself holds for it — plus the installed Claude Code plugins the project does NOT load and the `.trusty-mpm.toml` `[session] plugins` key that opts each one back in (issue #7422). Also compares the project's `.claude/settings.json` against the `enabledPlugins` map `prepare_session` would write and names every missing or divergent key: until issue #7678 the check reported the DECISION while a project that missed the write — a session paused before it existed, or resumed across the upgrade that added it — kept loading every user-tier plugin, with nothing on screen saying so. `Warn` when a plugin is excluded, the settings file does not carry the decision, or the protected `.claude.json` could not be read; `Ok` otherwise; never `Fail`. Read-only; `tm doctor --fix --yes` owns the write, re-applying both the project-tier `enabledPlugins` map and the composed session-MCP file through the launch path's own writers, and only for a project carrying a `.trusty-mpm/` marker.",
    ),
    (
        "tmux_options",
    ...
    (
        ...
        "The launchd `ProcessType` of the two tm jobs that start tmux servers — `com.trusty.mpm` and `com.trusty.mpm.supervisor`, read from `~/Library/LaunchAgents`. A tmux server inherits the class of the job that started it, and so does every session inside it: the supervisor plist declared `Background`, which held tmux, the PM sessions and their `cargo` gates at Darwin priority 4 on efficiency cores with throttled I/O, a clamp `taskpolicy -B` cannot lift (issue #8415). FAILS on `Background`; warns on any other value short of `Interactive`, including an absent key, which is launchd's throttled `Standard` default. Each finding names the `plutil -replace` and `launchctl bootout`/`bootstrap` commands for the plist on disk; a tmux server already running keeps its class until it exits. UNKNOWN when a plist exists but cannot be read as XML (a binary plist). `Ok` when neither plist is installed. Read-only.",
    ),
    (
        "tmux_priority",
        "The Darwin scheduling priority of the RUNNING tmux server, read with `tmux display-message -p '#{pid}'` and `ps -o pri= -p <pid>`. A server keeps the class it started with, so fixing a plist `launchd_process_type` flagged does not lift a server that is already clamped (issue #8415). FAILS below priority 20 — `Standard` launchd jobs run at 20, interactive shells at 31, and a `Background` job's tree at 4 — naming the server PID, the observed priority, and the remedy: fix the plist, then restart the tmux server (`tmux kill-server` ends every session in it; `tm` resumes them). `Ok` at 20 or above, and when no tmux server is running. UNKNOWN — never `Ok` — when tmux cannot be run, `ps` fails, or either answer cannot be parsed. Read-only.",
    ),
    (
        "pty_headroom",
    ...
    (
        "auto_memory",
        "Whether this project's auto-memory FALLBACK posture is correct (issue #7685). `trusty-memory` is the memory; Claude Code's own auto memory (`MEMORY.md`) stays on only while trusty-memory is unavailable (owner ruling 2026-09-12), and a managed launch turns it off two ways — the project-tier `autoMemoryEnabled: false` settings key and `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` on the spawned session — both written only when trusty-memory answered. The settings key is written TWO-WAY: when trusty-memory does not answer the launch writes `autoMemoryEnabled: true` instead, so a project an earlier healthy launch turned off gets its fallback back rather than keeping a `false` nothing could ever revert. Neither is observable afterwards, so this row reads the EFFECTIVE `autoMemoryEnabled` value across the same four layers Claude Code consults — project local, project, user local, user — names the layer it resolved from, reads whether the project's `MEMORY.md` still holds facts, and grades the pair against a live trusty-memory health probe. `Fail` when trusty-memory is healthy and auto memory is on (an unset key counts as on: Claude Code documents it ON by default), when a healthy trusty-memory sits beside an index that still holds facts, or when a settings file could not be parsed. `Warn` when trusty-memory is down and auto memory is OFF — this project has NO memory at all, the state the two-way write exists to prevent. `Ok` when trusty-memory is healthy, auto memory is off and the index is empty or absent, and equally when trusty-memory is down and auto memory is carrying the project — the fallback working as designed. Read-only; `tm doctor --fix --yes` writes the key into the PROJECT tier through the same merge the launch path uses, in the direction reachability dictates — `false` beside a healthy trusty-memory, `true` to restore the fallback beside one that is not answering — preserving every other key, and never rewrites a `settings.local.json` the operator owns. `--fix` never migrates — `tm memory import-auto-memory` stores each fact in the palace, archives the file, and only then empties the index, and the row names it.",
    ),
    (
        "ancestor_claude_md",
        "Every `CLAUDE.md`, `CLAUDE.local.md` and `.claude/CLAUDE.md` in a directory ABOVE the project root (issue #7673). Claude Code loads memory files from the session's cwd up to the filesystem root, so one file at `$HOME` is prepended to every turn of every agent in every project beneath it — and no other doctor row looks above the project, which is how a tm seed template sat at `$HOME` on 2026-09-12 costing every turn. The project root is the NEAREST enclosing directory holding a `.git` entry or a `.trusty-mpm` directory, whichever comes first walking up from the directory `tm doctor` runs in; `$HOME` itself is never a boundary, because `~/.trusty-mpm` is tm's global state directory and a dotfiles `.git` would otherwise swallow every project under it. A linked worktree nested inside its main checkout (Claude Code's `.claude/worktrees/<name>`) resolves to that checkout, which is the project identity Claude Code itself gives it; an independent clone and a submodule keep their own boundary. With no boundary, the start directory is the root. Reports each file's path, byte size and a token estimate (bytes/4, a rule of thumb, not a measurement). `Ok` when nothing above the root loads — including when every such file is already named in `claudeMdExcludes`, read as the union across the project, user and managed settings layers. `Warn` when a file with real content loads, and `Warn` naming the error when the scan cannot run at all (a directory that does not resolve, an ancestor probe failing for a reason other than a denied permission) — a scan that did not happen is never `Ok`. An ancestor directory that denies permission is skipped rather than failing the scan: the row is at least `Warn` and names each such directory as not checked. `Fail` when a file is tm's own seed template with no project content added. Read-only. `tm doctor --fix --yes` renames a pure seed template aside to `<name>.stale-seed-<YYYYMMDD>` (reversible, refused when that name exists) and adds a content-carrying file's absolute path to `claudeMdExcludes` in the resolved root's own `.claude/settings.local.json`, leaving the file untouched. The same scan drives a WARN at session launch and a stderr notice from `tm session instructions`. Nothing at or below the root is ever a finding or a target.",
    ),
];
