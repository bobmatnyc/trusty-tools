...
pub(crate) const DOCTOR_CHECKS: &[(&str, &str)] = &[
    ...
    (
        "auto_memory",
        "Whether Claude Code's own auto memory is off for this project (issue #7685). The owner directive is that auto memory (`MEMORY.md`) is not used in tm sessions — `trusty-memory` is the memory — and a managed launch makes that true two ways: the project-tier `autoMemoryEnabled: false` settings key, and `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` on the spawned session. Neither is observable afterwards, so this row reads the EFFECTIVE `autoMemoryEnabled` value across the same four layers Claude Code consults — project local, project, user local, user — and names the layer it resolved from. `Ok` for `false`; `Fail` for `true`, or for a settings file that could not be parsed; `Warn` when no layer sets the key at all, because Claude Code documents auto memory as ON by default, so an unset key is the directive unmet rather than a pass. Read-only; `tm doctor --fix --yes` writes `false` into the PROJECT tier through the same merge the launch path uses, preserving every other key, and never rewrites a `settings.local.json` the operator owns.",
    ),
];
