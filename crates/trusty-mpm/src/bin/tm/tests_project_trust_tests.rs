...
/// #7757: the hint tm prints must be a command tm's own parser accepts.
///
/// Why: every surface that told an operator to run `tm project trust <path>`
/// printed a positional form `ProjectAction::Trust` rejects with exit 2. Feeding
/// the emitted string back through `Cli::try_parse_from` is the only check that
/// stays true when either side changes — modelled on
/// `doctor_worktree_remediation_command_parses`.
#[test]
fn trust_command_hint_parses_as_the_cli_accepts_it() {
    let hint = trusty_mpm::core::project_trust::trust_command_hint(std::path::Path::new("/work/p"));
    assert_eq!(hint, "tm project trust --dir /work/p");
    let cli = Cli::try_parse_from(hint.split_whitespace())
        .unwrap_or_else(|e| panic!("the trust hint `{hint}` does not parse: {e}"));
    match cli.command.unwrap() {
        Command::Project {
            action: ProjectAction::Trust { dir, revoke },
        } => {
            assert_eq!(dir.as_deref(), Some("/work/p"));
            assert!(!revoke, "a grant hint must never revoke");
        }
        other => panic!("expected project trust, got {other:?}"),
    }
}
