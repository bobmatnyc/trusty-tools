...
## How the Snapshot Reaches the Default Branch (#7282)
...
`chore/sessions-<session>-<timestamp>` branch cut from the project's default
branch, pushes it, and opens a docs-only PR through `tm pr open` with squash
auto-merge armed. The result carries `snapshot_publish` with the branch, commit,
and PR URL; report the URL alongside `snapshot_path`.

The default branch is the one `config.yaml`'s `projects:` entry declares, else
whatever `origin/HEAD` points at, else `main` — so a `master` or `develop`
project publishes from its own branch rather than being told every pause is
illegal.
...
- **Not on the default branch.** Publishing from a feature branch would put
  unrelated commits in the PR, so the tool refuses and names the branch it
  expected. Commit and open the PR by hand.
...
machine-local: no commit, branch, or PR is created. Every no-PR outcome reports
`snapshot_publish.status: "skipped"` with a `reason` saying which — `not_tracked`
(git-ignored sessions), `not_a_git_repo` (the project directory is not a
checkout), or `unchanged` (the snapshot already matches the default branch).
...
Then **load the tool schema before calling it** — see `/tm-session-management`
("MCP Session-Tool Failure Diagnosis") for why absence from your loaded tool
list does not mean the tool is unavailable:
...
Never report a bare "MCP pause tool unavailable." Follow the shared diagnosis
procedure in `/tm-session-management` ("MCP Session-Tool Failure Diagnosis")
— it is not optional, and no step in it may be skipped or reordered. Only
once that procedure has established that no `mcp__trusty-mpm__*` tool is
available, hand-write the snapshot as a fallback — and label it as one (see
"Self-Identifying Fallback Snapshot" below). This is the only point at which
writing a file by hand is permitted.

When you do fall back, match the tool's own format exactly, not free prose —
reuse the shape from "Snapshot Content" above verbatim: RFC3339
