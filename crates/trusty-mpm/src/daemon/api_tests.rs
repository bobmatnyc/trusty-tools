...
async fn doctor_endpoint_returns_report() {
    // `GET /api/v1/doctor` returns a fourteen-check report (#1840 added the
    ...
    // the agent_skills check). #1905's stale-skill cleanup is a one-time
    // migration, not a `run_doctor` probe, so it does not appear here; the
