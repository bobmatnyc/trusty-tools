...
// `ResumeQuery` (issue #3981 Part 2) lives in `guard_flags.rs` — see that
// module for the query-parameter doc — to keep this already-oversized file
// from growing further; it is used here only via `guard_flags::ResumeQuery`.
...
pub async fn resume_managed_session(
    State(state): State<Arc<DaemonState>>,
    AxumPath(id_str): AxumPath<String>,
    axum::extract::Query(q): axum::extract::Query<guard_flags::ResumeQuery>,
) -> impl IntoResponse {
    let id = match parse_id(&id_str) {
        Ok(id) => id,
        Err((code, msg)) => return (code, msg).into_response(),
    ...
    match resume_managed(&state, &id).await {
        Ok(mut final_record) => {
            ...
            // `guard_flags::ResumeQuery`'s doc for why this is unconditional, and
            // `guard_flags::persist_after_resume`'s doc for the extraction
            // rationale (this file is already at its SLOC budget).
            guard_flags::persist_after_resume(
                &state,
                &mut final_record,
                q.disable_hooks,
                q.pm_unrestricted,
