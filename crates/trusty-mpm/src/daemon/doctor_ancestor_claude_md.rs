...
pub(super) fn check_ancestor_claude_md(
    ...
    // #7673: `project` is the CLI's process cwd, which may be any subdirectory;
    ...
    let found = match crate::core::ancestor_claude_md::scan(project, home, managed_config_dir) {
        ...
        Err(err) => {
            return DoctorCheck::new(
                ...
                CheckStatus::Warn,
                format!(
                    "could not check for CLAUDE.md files above {}: {err}. Ancestor memory \
                     files were NOT checked; fix the path or its permissions and re-run",
                    project.display()
                ),
            );
        }
...
mod tests {
    ...
    /// FAILS AGAINST ROUND 3 (#7673): a directory that does not exist resolved
    /// to itself, scanned nothing, and reported `Ok` — a fail-open on the one
    /// check that exists to find stray memory files.
    #[test]
    fn a_missing_project_directory_warns_not_ok() {
        let (_tmp, ancestor, _project) = fixture();
        let missing = ancestor.join("does-not-exist");

        let check = check_ancestor_claude_md(Some(&missing), None, None);
