...
// #7685: `CheckStatus`/`DoctorCheck` moved out with the sidecar probes — this
// file registers checks other modules construct, and no longer builds one.
...
// #7685: the trusty-memory / trusty-search health probes and their retry,
// classification and index helpers. Split out because this file is the check
// REGISTRY and was sitting at the 500-SLOC production cap — see that module's
// own doc. `pub(crate)` because `PROBE_TIMEOUT` is part of the daemon's
// probe-budget vocabulary; the checks themselves stay `pub(super)`.
#[path = "doctor_sidecars.rs"]
pub(crate) mod doctor_sidecars;
/// Re-exported so `doctor_search_pin` keeps naming one probe budget (#7685).
pub(crate) use doctor_sidecars::PROBE_TIMEOUT;
use doctor_sidecars::{check_memory, check_search};
...
/// Test: `run_doctor_produces_fifty_nine_checks`,
...
/// Resolve the two host facts the `auto_memory` row needs, then grade it.
///
/// Why (#7685): the check itself is a pure function of its inputs so its three
/// verdicts are testable, which leaves someone to resolve those inputs from the
/// host. Doing it here keeps the registration above a single line — two other
...
/// this wiring is covered by `run_doctor_produces_fifty_nine_checks`.
