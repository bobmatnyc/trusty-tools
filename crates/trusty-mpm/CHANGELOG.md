...
- per-project GitHub account pinning: an optional `gh_account` field on the project registration/config (`Project`/`ProjectConfig`, register/patch routes, `tm projects register --gh-account`) lets an operator pin which `gh`-authenticated account a project's sessions use. At every managed session spawn/relaunch, `core::gh_account::resolve_gh_account_env_for_workspace` detects the workspace's git origin, matches it against the registered project, and — when `gh_account` is set — resolves a token via `gh auth token -u <account>` and injects `GH_TOKEN`/`GH_USER` into the spawned Claude Code process's environment, so `gh` identity is deterministic for that session's Bash-tool `gh` calls regardless of the host's global `gh auth switch` state. Resolution is fail-open: a missing `gh` binary, an account that isn't logged in, or any other failure logs a warning and the spawn proceeds without `GH_TOKEN` — it never blocks the session. Only the account NAME is ever persisted; the token is resolved fresh from the keyring at spawn time and never written to disk or logged (closes #3025)
