...
- per-project GitHub account pinning: an optional `gh_account` field on the project registration/config (`Project`/`ProjectConfig`, register/patch routes, `tm projects register --gh-account`) lets an operator pin which `gh`-authenticated account a project's sessions use. At every managed session spawn/relaunch, `core::gh_account::resolve_gh_account_env_for_registry` detects the workspace's git origin and matches it against the `ProjectRegistry` (the actual write target for `tm projects register`/the PATCH route/the MCP tool, resolved off the async executor via `tokio::task::spawn_blocking`), and — when `gh_account` is set — resolves a token via `gh auth token -u <account>` and delivers `GH_TOKEN`/`GH_USER` to the spawned Claude Code process via a mode-0600 temp file the pane shell sources and immediately deletes (never as a literal value in the command line sent via `send-keys`, keeping it out of shell history and `ps` output), so `gh` identity is deterministic for that session's Bash-tool `gh` calls regardless of the host's global `gh auth switch` state. Resolution is fail-open: a missing `gh` binary, an account that isn't logged in, or any other failure logs a warning and the spawn proceeds without `GH_TOKEN` — it never blocks the session. Only the account NAME is ever persisted; the token is resolved fresh from the keyring at spawn time and never written to disk longer than the instant between write and source, nor logged (closes #3025)
- custom (non-native) MCP server bridging into daemon-managed fleet sessions, at USER and PROJECT scope (issue #2739 follow-up): previously only the trusty-family native allowlist (`slack-mcp`/`telegram-mcp`/`gworkspace-mcp`/`trusty-analyze`) was bridged from the `tm mcp add` registry into a fleet session's `.mcp.json` — any other server (e.g. a remote Duetto HTTP endpoint) was invisible to `tm session new` sessions. A new `session_launch::custom_mcp` injector now bridges EVERY user-scope registry entry that isn't native/builtin, plus a new `[mcp.custom.<name>]` table in the existing per-project `<project>/.trusty-mpm/manifest.toml` override file (project scope; overrides a same-named user-scope entry). Custom stdio servers reuse the native module's `env`-to-`.env.local` secret routing (never written into the git-tracked `.mcp.json`); a custom remote (http/sse) server bridges only when it declares no `headers` — a headers-bearing remote entry is rejected (fail-closed) pending a decision on safe header-secret delivery (closes #2739). **Security hardening (issue #3033, pre-release review):** the project-scope loop previously skipped the reserved-name check the user-scope loop already applied, letting a project manifest silently overwrite a legitimate `trusty-memory`/`trusty-search`/native-allowlist entry — both loops now reject reserved names identically. Project-scope `[mcp.custom]` entries additionally require explicit operator consent via the new `tm project trust <dir>` / `tm project trust --revoke <dir>` CLI verb (`core::project_trust`, state recorded under `~/.trusty-tools/trusty-mpm/project-trust.json` — never inside the repo, so a cloned repo can never self-trust); an untrusted project's `[mcp.custom]` table is skipped entirely. As defense-in-depth, a trusted project's bridged names are still excluded from `enabledMcpjsonServers` auto-approval, so Claude Code's own "new MCP servers found" consent dialog still fires for them in-session. **Known limitation (tracked, not fixed here):** neither scope prunes a bridged `.mcp.json` entry when its source registry/manifest entry is later revoked or removed — see the follow-up issue for the tracked-name-set pruning approach. **Further hardening (issue #3033, code-critic pre-merge review, folded into the same PR):** every custom server name (both scopes) must now match `^[a-z0-9][a-z0-9-]*$` — `inject_one_custom_entry` rejects anything else via the new `is_valid_custom_mcp_name`, closing the case-variant (`Trusty-Memory`) and Unicode-confusable (e.g. a non-ASCII hyphen) typosquat class that the exact-string `is_reserved_name` check alone did not catch. `core::project_trust`'s module docs and the `tm project trust` CLI help now state explicitly that trust is per-DIRECTORY (a canonicalized path), not per-repo-content — replacing a trusted directory's contents (re-clone in place, checkout to a different remote/branch) silently inherits the existing grant; re-clone to a new path or `--revoke`-then-re-trust after replacing content (a future git-remote/commit-bound hardening is tracked in #3051). `project-trust.json` is now written with owner-only `0o600` permissions on Unix
...
- bridged custom MCP entries (`session_launch::custom_mcp`) accumulate in `.mcp.json` forever — revoking a `tm mcp add` registration or removing an `[mcp.custom]` manifest entry does not remove the corresponding entry from a workspace's `.mcp.json` on the next `prepare_session`. Tracked in #3045 (tracked-name-set pruning approach).

### Fixed

- Two remaining `claude`-spawning call sites now disclaim macOS TCC responsibility, closing the recurring "trusty-mpm would like to access data from other apps" prompt storm on the `tm run`/`tm login` and daemon actor-managed session paths (#2819/#2721/#2820 fixed only the tmux-hosted managed-session path). `standalone::run::build_launch_command`/`build_login_command` (the `tm run <alias>`/`tm login` interactive drivers) now spawn via the new `disclaimed_status`, and `control::backend::stream_json::StreamJsonBackend` (the daemon's default actor-managed session backend — the path an agent's own `cargo build` runs under) now spawns its long-lived piped `claude -p --output-format stream-json` child via the new `disclaimed_piped_spawn`, so an agent-run build script touching `~/Library/CloudStorage` is attributed to `claude`'s own stable code identity instead of rolling up to the signed `trusty-mpm` binary. `core/spawn_disclaim.rs` is split into a module (`mod.rs` + `macos/{capture,status,piped}.rs`) to keep each spawn shape under the 500-SLOC cap. Non-macOS behavior is unchanged (closes #2997)
- `write_project_hooks` (the daemon's managed-launch project-tier `.claude/settings.json` writer) now writes via `trusty_common::claude_config::write_json_atomic` (temp-file + rename, with a `.bak` backup of the prior content) instead of a plain `fs::write`, matching the atomic-write convention already used by the sibling `tm hooks clean` mutation path. Since #2969 this function writes the full lifecycle triad on every managed-session launch, so a crash or `^C` mid-write previously risked leaving a truncated `settings.json`; the write mechanism now can no longer tear (closes #2972)
- `core::tmux`'s pure argv-construction layer (`TmuxTarget`, `TmuxCommand`, `tmux_argv`, `scrollback_option_commands`, `DEFAULT_TMUX_HISTORY_LIMIT`/`DEFAULT_TMUX_MOUSE`) moved to `trusty-common::tmux` as the workspace-wide single source of truth (issue #3004) — a scrollback bug report against trusty-agents' independent tmux implementation (which never got the #2398/#2399 history-limit-before-`new-session` fix) surfaced the real root cause: two crates were maintaining separate, drifting tmux command layers. `core::tmux` is now a thin re-export/adapter; `create_managed_session`'s behavior, config resolution, and TCC-disclaimed spawning are unchanged (closes #3004, refs #2398, #2399)
- `tm-cli-operations` bundled skill now explicitly teaches user-scope MCP registration: an ANTI-PATTERN callout tells a PM/agent never to hand-edit a project's `.mcp.json` to register a user-wide server (use `tm mcp add`, the true "available in all your projects" scope, instead), and a new propagation-caveat subsection documents that an already-running managed session does not hot-reload a newly `tm mcp add`-ed server — it must be relaunched (`tm session resume`) to pick it up, and for fleet sessions (`tm session new`) only the native allowlist (`slack-mcp`/`telegram-mcp`/`gworkspace-mcp`/`trusty-analyze`/`trusty-memory`/`trusty-search`) bridges at all (#2739). Fixes a real mis-delegation where a PM asked to "register the duetto-memory MCP" had an agent edit a project-level `.mcp.json` instead (closes #3020)
- The bare `tm` guided default (no subcommand) no longer silently auto-starts or reconnects to a DIFFERENT daemon when an explicitly-supplied `--url` / `TRUSTY_MPM_URL` is unreachable — it now probes the explicit URL up front and exits 75 with a message naming the failed URL, instead of falling through to `ensure_daemon_started`'s implicit lock-file/default re-resolution and silently operating against whatever real daemon happens to be running. Implicit resolution (no `--url`/`TRUSTY_MPM_URL` set) and `tm hook`'s fail-open contract are both unaffected. `resolve_daemon_url_probing` (library-level) and the new `resolve_daemon_url_for_cli` helper now return a `DaemonUrlError::Unreachable` instead of falling back for an explicit, unreachable URL; the exit code (75) is a shared constant also used by the pre-existing `tm session prune-idle` SM-unavailable exit code (closes #1737)
- `tm doctor`'s `output_style` check validated resolution only (configured id maps to a non-empty file) and missed content drift — a deployed style file could carry stale text (e.g. an already-corrected PM identity string) through an install + daemon restart while `tm doctor` still reported `Ok`. Adds a new `output_style_staleness` check that byte-compares each deployed `~/.claude/output-styles/*.md` file against the bundled catalog and `Warn`s on drift (pointing at `tm install`), plus orphan-file detection for foreign/dormant files in the same directory (e.g. a pre-rebrand `claude-mpm.md`) that match no bundled style and are not the currently configured id — flagged, never auto-deleted, bringing the doctor suite to 18 checks (closes #2333)
...
- `tm hooks clean` / `tm doctor`'s `hooks_contamination` and `hooks_foreign_conflict` checks now detect a hand-mixed hook GROUP (one tm-owned command alongside one foreign command in the same matcher group) — the previous whole-group `.all()` classification made such a group invisible to both, so a live tm entry could survive `tm hooks clean --force` undetected. Detection and removal now operate at per-ENTRY granularity: `tm hooks clean --force` strips only the tm-owned entry, leaving the foreign entry and the group itself intact (closes #2948). The daemon's managed-launch path (`claude --setting-sources project,local`, which excludes the `user` tier) now also provisions the full trusty-mpm lifecycle hook triad (circuit breaker / audit log / dashboard observability) into the PROJECT-tier `.claude/settings.json` alongside the existing `trusty-memory` + PM-guard hooks — previously the triad was written only into the user-tier `CLAUDE_CONFIG_DIR` settings.json, which managed sessions never load, so the daemon was blind to their lifecycle events. The project-tier writer now merges (rather than replaces) the `hooks` key, so a project's own pre-existing foreign hooks survive every relaunch (closes #2003)
...
- `pm_guard` per-turn file-change budget + content-aware delegation routing (closes #2918, refs #2745): a PM source-code Edit/Write or shell-based file edit (`sed`/`awk`/`patch`/`git apply`/redirection) is no longer an absolute single-call prohibition — up to 3 combined file changes are allowed per turn-window before the hook hard-blocks with a message stating the exact budget count used. The budget is tracked via a short-TTL counter file under `<HOME>/.trusty-mpm/state/pm_guard_turn_budget/` (no true per-turn hook signal exists yet — see `commands::pm_guard_budget` for the documented heuristic). Denial messages that suggest delegation now route by the target file's content type (`.md`/`docs/` → the documentation agent, `.rs` → rust-engineer, other mainstream languages → their matching stack engineer, else a generic engineer-agent hint) instead of hardcoding `rust-engineer` (`commands::pm_guard_routing`). Also fixes a `2>/dev/null`/`&>/dev/null`/`2>&1` read-only-redirection false positive Bob hit live in another session — confirmed already correct on `main` for the exact repro command, now locked in with a permanent regression test.
