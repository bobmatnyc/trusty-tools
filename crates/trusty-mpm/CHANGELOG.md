...
- `core::instruction_package`: the sectioned-JSON instruction-package schema
  ...
  *before* the package is deserialized, so a package from a later schema reports
  ...
  A declared section must own at least one *non-optional* block. Owning blocks
  is not the same as emitting: a section covered only by `optional` blocks
  passed validation and still composed to nothing, recreating the very
  silent-missing-section shape the coverage check exists to prevent.
