...
- `resolve_daemon_url_probing` no longer silently falls back to the daemon lock file / compiled-in default when an explicitly-supplied `--url` / `TRUSTY_MPM_URL` fails its reachability probe — it now returns a `DaemonUrlError::Unreachable` naming the failed URL instead, with a documented exit code (`EXIT_DAEMON_URL_UNREACHABLE`, 75). Implicit resolution (no `--url`/`TRUSTY_MPM_URL` set) is unaffected and keeps the existing gateway → lock file → default fallback chain. A new `resolve_daemon_url_for_cli` helper gives any future caller wanting the strict "explicit URL must be reachable" contract a single entry point (closes #1737)
