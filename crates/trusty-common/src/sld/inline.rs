...
use super::grammar::{has_traversal, reference_regex};
...
/// the result to *declared* linkage (§2.3, §2.4).
...
/// to `Spec References`) opens a block; a different heading or a non-comment line
/// closes it; fenced regions are skipped entirely. Within an open block, the
/// §2.2 reference regex extracts each `(id, path, anchor)` triple, dropping any
/// path with a `..` traversal segment. Duplicates are retained (each keeps its
/// own line) so a linter can flag each occurrence.
/// Test: `super::tests::inline_rust`, `inline_bare_shell`, `inline_skips_fenced`,
/// `inline_ignores_non_block_ref`.
#[must_use]
