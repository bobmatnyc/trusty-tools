...
- tool-calling dream-consolidation PoC (epic [#2866](https://github.com/bobmatnyc/trusty-tools/issues/2866)): new `memory_core::dream_consolidation` module — one `emit_consolidation` tool call per cluster via the existing `ChatProvider` surface returns `{summary, inferences[], facts[]}`; summary stored as a tagged drawer, facts asserted with `dream:llm_cluster_consolidation` provenance, and source drawers tombstoned (never deleted) via `superseded_by` triples written LAST. Cluster v0 = room snapshot. Default OFF (`DreamConfig::consolidation.enabled = false`) and fail-open: disabled/no-provider/provider-error all degrade to zero-effect stats and the dream cycle completes unchanged
- recall-time tombstone filter (epic #2866): default `recall` (L0+L1+L2) now excludes drawers with an active `superseded_by` KG edge, preloaded via the new bulk `KnowledgeGraph::subjects_for_predicate` / `archived_drawer_ids` helpers (fail-open — a KG error hides nothing); `recall_deep` deliberately does NOT filter and remains the include-archived path. The legacy `semantic_consolidation_pass` / `consolidate_scoped` snapshots also exclude archived drawers so tombstoned originals are never re-clustered
