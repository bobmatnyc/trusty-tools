...
- Fix round (#7154): `prune_confirmed` now refuses a confirmed prune batch outright — deleting nothing that tick — when it exceeds `PRUNE_BATCH_COUNT_CEILING` (500 entries) or more than `PRUNE_BATCH_FRACTION_CEILING` (50%) of a manifest over `PRUNE_FRACTION_FLOOR` (20) entries, reporting the refusal via the new `PruneOutcome::refused`/`PruneRefusal`/`PruneRefusalReason`. Also: `FailingDestination` in the test suite now injects a `delete` failure alongside its existing `put` injection, proving a failed delete keeps both the object and its manifest entry.
...
- The `search-index` feature is now unix-only, and says so (#7237). `search_index`, `index_dispatch` and `search_readiness` are gated on `all(unix, feature = "search-index")` alongside the socket client they reach for; enabling the feature on any other target is a `compile_error!` naming the reason instead of an unresolved `crate::search_rpc`.
...
- `workspace_layout`'s module doc no longer cites `provisioner::workspace::provision_in` as a worktree creation site — ADR-0055 deleted it ([#6000](https://github.com/bobmatnyc/trusty-tools/issues/6000)). `inproject::worktree_path_for` is the remaining one. No behavior change.
