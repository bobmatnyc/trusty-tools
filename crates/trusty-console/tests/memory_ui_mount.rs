//! The trusty-memory SPA mount at `/tools/memory/` (#6155).
//!
//! Why: #6286 deleted trusty-memory's HTTP surface, and the `/` mount that
//! served this dashboard with it — the bundle stayed embedded in a daemon with
//! no listener. This mount is where it moves, so it has to be proven reachable:
//! shell, hashed assets, and the injected API base that repoints every call at
//! the console's `/api/memory/` bridge.
...
#[tokio::test]
async fn memory_ui_bare_path_redirects() {
    let (status, headers, _) = get("/tools/memory").await;
    ...
    assert_eq!(location, "/tools/memory/");
}

#[tokio::test]
async fn memory_ui_index_serves_the_spa_with_the_bridge_base_injected() {
    let (status, headers, body) = get("/tools/memory/").await;
    ...
    assert!(body.contains("<title>Trusty Memory</title>"), "{body}");
    assert!(
        body.contains(r#"window.__MEMORY_BASE__ = new URL("/api/memory/", document.baseURI)"#),
    ...
    let injected = body.find("__MEMORY_BASE__").expect("global present");
...
#[tokio::test]
async fn memory_ui_serves_every_asset_the_shell_references() {
    let (_, _, shell) = get("/tools/memory/").await;
...
#[tokio::test]
async fn memory_ui_unknown_path_falls_back_to_the_shell() {
    let (status, _, body) = get("/tools/memory/no/such/file").await;
    assert_eq!(status, StatusCode::OK);
    assert!(body.contains("<title>Trusty Memory</title>"));
}

/// Why: two mounts serving from one binary is the whole risk this file exists to
/// close — a wrong `SpaMount` constant would serve one dashboard at the other's
/// URL, and every API call would then reach the wrong daemon.
/// Test: this is the test.
#[tokio::test]
async fn the_two_tool_mounts_do_not_shadow_each_other() {
    let (_, _, memory) = get("/tools/memory/").await;
    let (_, _, search) = get("/tools/search/").await;
    assert!(memory.contains("Trusty Memory") && !memory.contains("Trusty Search"));
    assert!(search.contains("Trusty Search") && !search.contains("Trusty Memory"));
    assert!(memory.contains("__MEMORY_BASE__") && !memory.contains("__SEARCH_BASE__"));
    assert!(search.contains("__SEARCH_BASE__") && !search.contains("__MEMORY_BASE__"));
}
...
async fn console_spa_still_owns_its_own_routes() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert!(
        !body.contains("<title>Trusty Memory</title>"),
        "/ must still serve the console SPA, not the memory one"
    );
}
