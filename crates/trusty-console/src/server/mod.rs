...
//!   - `ANY /api/memory/{*path}` — the same for trusty-memory (#6155); see
//!     `crate::memory_uds`.
...
//!   - `GET /tools/memory/*path` — the same for the trusty-memory SPA (#6155).
//!
...
pub struct AppState {
    ...
    /// Override for the trusty-memory socket path (#6155).
    ///
...
// ─── router ─────────────────────────────────────────────────────────────────
//
// #6155: the router moved to `router.rs` when the memory dashboard's five
// routes pushed this file over the 500-SLOC cap. The re-export keeps every
// call site — `main`, `lib::run_serve`, and every test — unchanged.
mod router;
pub use router::{build_router, build_router_with_self_origins, build_router_with_webhooks};
...
async fn services_handler(State(state): State<AppState>) -> axum::response::Response {
    ...
    if let Some(snap) = state.poller_cache().snapshot().await {
        ...
        crate::service_metrics::apply_metrics_overlay(&mut services, state.machine_history()).await; // #6642
    ...
    .await
    {
        Ok(mut infos) => {
            ...
            crate::service_metrics::apply_metrics_overlay(&mut infos, state.machine_history())
                .await; // #6642
