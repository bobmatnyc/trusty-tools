<script lang="ts">
  ...
  // (`StartWorkingForm`, directly above this card in `WorkstreamTab.svelte`
  // — no separate navigation affordance needed, it's already on screen).
  //
  ...
  // workstream, then — only when one is active — `GET /sessions` +
  ...
  // when `EventSource` is unavailable or no workstream is active. Renders
  // one of four phases (`connecting`/`daemon-unreachable`/`no-workstream`/
  // `ready`), same never-hidden discipline as `StatusBar` (DOC-39 AC-4.2);
  // `ready` itself branches on whether a bound session was found. A `GET
  // /sessions/{id}` 404 immediately after a successful list (session
  // vanished mid-poll) degrades to the session-level sub-empty-state, not
  // `daemon-unreachable` — the daemon answered fine. Cancel requires a
  // two-step in-card confirm (no `window.confirm()`) and forces an
  // immediate `refresh()` on success rather than waiting for the next poll
...
<section class="mt-4 rounded border-1.5 border-trusty-border bg-trusty-card">
  ...
  <div class="p-4">
    ...
    {:else if activeWorkstream}
      <p class="font-mono text-[11px] uppercase tracking-wide text-trusty-text-secondary">
        {workstreamLabel(activeWorkstream)}
      </p>

      {#if !session}
        <p class="mt-2 flex items-center gap-1.5 text-xs text-trusty-text-muted">
          <span class="h-1.5 w-1.5 rounded-full bg-status-neutral"></span>
          no activity yet
        </p>
      {:else}
