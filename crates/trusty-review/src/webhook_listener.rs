...
//! ownership, and exits on SIGTERM. Draining the inbox into the review pipeline
...
use trusty_common::webhook_relay::{ListenerError, WebhookListener, review_socket_path};
...
pub fn inbox_root() -> anyhow::Result<PathBuf> {
    Ok(trusty_common::resolve_data_dir("trusty-review")?.join(INBOX_DIR_NAME))
}
