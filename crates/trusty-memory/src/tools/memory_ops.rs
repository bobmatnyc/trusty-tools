...
//!
//! Why: the `memory_*` write/list/forget handlers (remember/note/list/forget/
//! send_message) form one cohesive group split out of the former monolithic
//! `tools.rs` (issue #607). The recall family left for
//! [`super::recall_ops`] when this file crossed the 500-SLOC production cap
...
//! `dispatch_memory_list_*` in `tools::tests`.
...
use trusty_common::memory_core::retrieval::{list_drawers_in_wing, RecallScope, RememberOptions};
use uuid::Uuid;

use super::bm25::bm25_delete_document;
use super::helpers::{
...
// `memory_list` takes the same `room`/`wing` scope the recall tools do
// (ADR-0027 D4.1), so it borrows the resolver rather than re-spelling it.
use super::recall_ops::recall_scope;
