...
impl LockStallTracker {
    ...
    /// Sweep every open palace now.
    pub fn sweep(self: &Arc<Self>, registry: &PalaceRegistry) {
        self.sweep_at(registry, Instant::now());
    }
    ...
    ///
    /// What: a free lock removes only a stamp with no live probe. A live
    /// probe's stamp is left for the probe, which may not yet have queued. A
    /// held lock gets a stamp (an existing one keeps its older `since`) and a
    /// spawned probe unless one is already queued. Without a Tokio runtime the
    /// stamp is kept unprobed, and the next sweep retries.
    ...
    pub(crate) fn observe(
        ...
        if let Ok(free) = mutex.try_lock() {
            ...
            if stalls.get(&key).is_some_and(|s| !s.probing) {
                stalls.remove(&key);
            }
            return;
        }
        let Some(token) = self.claim(key, now) else {
            return;
        };
    ...
    /// Stamp `key` as probing, returning the probe's token, or `None` when a
    /// probe is already queued.
    pub(crate) fn claim(self: &Arc<Self>, key: StallKey, now: Instant) -> Option<ProbeToken> {
        let mut stalls = self.guard(&self.stalls);
        match stalls.get_mut(&key) {
            Some(s) if s.probing => return None,
            Some(s) => s.probing = true,
            None => {
                stalls.insert(
                    key.clone(),
                    Stall {
...
impl ProbeToken {
    /// Queue on `mutex` until it is acquired, then clear the stamp.
    pub(crate) async fn wait(mut self, mutex: Arc<tokio::sync::Mutex<()>>) {
        drop(mutex.lock().await);
        self.tracker.guard(&self.tracker.stalls).remove(&self.key);
        self.acquired = true;
    }
}
...
impl Drop for ProbeToken {
    fn drop(&mut self) {
        ...
        if let Some(s) = stalls.get_mut(&self.key) {
            s.probing = false;
        }
    }
}
...
/// What: beats, sleeps `interval`, beats, sweeps, forever. A panicking sweep
/// ends the task, and the stale heartbeat then reads as degraded.
