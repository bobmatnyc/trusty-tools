...
//! stay byte-identical.
//!
//! Test: `tests` below assert the tool count (34), the name string, and
...
/// `ServiceDescriptor` impl that advertises this crate's 34 memory tools.
///
...
mod tests {
    ...
    fn tools_returns_expected_count() {
        ...
        assert_eq!(
            tools.len(),
            34,
            "expected 34 memory tools (including chat-session + dream-ops + palace_dream), got {}",
            tools.len()
        );
    }
