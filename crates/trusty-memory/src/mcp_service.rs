...
//! stay byte-identical.
//!
//! Test: `tests` below assert the tool count (37), the name string, and
...
/// `ServiceDescriptor` impl that advertises this crate's 37 memory tools.
///
...
mod tests {
    ...
    fn tools_returns_expected_count() {
        ...
        assert_eq!(
            tools.len(),
            37,
            "expected 37 memory tools (including chat-session + dream-ops + palace_dream), got {}",
            tools.len()
        );
    }
