//! `memory.health` and the probe helpers behind it.
//!
//! Why this file exists (#6286): these tests lived in `web::tests::health_tests`
//! and drove an in-process axum router. The router is gone, but almost nothing
//! here was ever about HTTP — the probe tests call
//! `run_health_round_trip_inner` and `seed_probe_sentinel_if_absent` directly,
//! and the rest only wanted the handler's JSON body. `health_body` is where the
//! transport was, so `health_body` is the only thing that changed: it calls the
//! handler instead of routing to it.
//!
//! What is gone with the router and NOT ported: nothing. Every assertion here
//! survives; `rpc_health_answers_over_a_real_socket` in
//! `crate::transport::uds::tests` covers the wire half the router used to.
...
/// Run the cheap (non-probe) health path and return its body.
///
/// #6286: this used to build the axum router and `GET /health`. The handler is
/// now a plain async function, so the whole transport hop is one call — and the
/// `assert_eq!(status, OK)` it replaced is gone because there is no status code
/// to assert. A handler failure surfaces as the `expect` below.
