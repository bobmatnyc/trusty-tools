//! Daemon event types and activity-log helpers for trusty-memory.
//!
//! Why: `lib.rs` was over the 500-SLOC production file cap (issue #607).
//! `HookType`, `InjectionKind`, `DaemonEvent`, and the
//! `open_activity_log_with_fallback` helper are a self-contained unit — they
//! form the event-bus vocabulary used by handlers, the activity log, and the
//! SSE stream — so extracting them here keeps `lib.rs` under the cap while
//! preserving all public API.
//! What: exports `HookType`, `InjectionKind`, `DaemonEvent`, and
//! `HOOK_PROMPT_EXCERPT_CHARS`/`hook_prompt_excerpt` (moved here to co-locate
//! with the event types that depend on the excerpt). Also exports
//! `open_activity_log_with_fallback` as `pub(crate)`.
//! Test: `lib_tests` covers `hook_type_serde_round_trips`,
//! `injection_kind_serde_round_trips`, `daemon_event_type_str_matches_sse_tag`,
//! and the excerpt helpers.

use std::path::Path;
use std::sync::Arc;

use crate::activity::{ActivityLog, ActivitySource};
