...
- `transport::uds`'s serving half is gated behind the `daemon` feature. It registers `memory.chat`, whose handler lives in the `daemon`-gated `crate::chat`, so a `default-features = false` library build (the one `trusty-agents` links `MemoryMcpService` through) did not compile at all. `socket_path`, `METHOD_HEALTH`, `FOLDED_METHODS` and `STREAM_METHODS` stay ungated — that is what a client needs
