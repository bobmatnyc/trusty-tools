...
//! compiles and warns loudly. After a successful build it re-stamps
//! `ui/dist/ui-source-hash.txt`, which Vite's `emptyOutDir` has just deleted
//! (#5936).
//!
...
fn main() {
    ...
    if build_svelte_ui(&ui_dir, &dist_dir, "trusty-memory") {
        restamp_ui_bundle(&crate_root, "trusty-memory");
    }
}
