...
- **Error response**: any 4xx / 5xx returns a JSON body whose `error` field is a
  ...
  **Index-scoped error contract (#5061, #5068).** Every index-scoped endpoint
  ...
  | `index_corpus_unavailable` | 503 | yes/no | The durable corpus cannot answer. Two producers, one field set: it failed to OPEN (#4087, `failure_kind` per the #4333 classification, `retryable` mirroring `transient`), or it opened and then failed a READ (#6043, #5917 — `failure_kind: "read_failed"`, `retryable: true`). Every corpus-backed endpoint reports a read failure this way: `search`, `chunks`, `grep` (per-index and global), and `call_chain`. Each used to answer it as something else — `200 {"results": []}`, an empty page, `200 {"matches": [], "total": 0}`, and `404 entry point not found` for a symbol that exists. The global `POST /search` fan-out is the one exception by design: it still answers `200` so one broken index cannot fail the sweep, and reports the drop as `corpus_read_failed_indexes_skipped` (the read-failure sibling of `corpus_failed_indexes_skipped`). |
  | `index_migration_in_progress` | 503 | `true` | A schema migration is rebuilding this index's corpus (#6581). M005 empties the corpus and re-chunks it in batches, so every read still succeeds and simply finds nothing — answering would report a mid-rebuild corpus as a genuine miss. Carries `failure_kind: "migrating"` and `transient: true`. Reported by `search`, `chunks`, `grep` (per-index and global), `call_chain`, `graph/neighbors`, and the `graph` ingest, which refuses before it persists. `GET /indexes/:id/status` and `GET /health` deliberately still answer, so a caller can find out why the rest are refusing. The global `POST /search` fan-out also still answers `200` and reports the drop as `migration_in_progress_indexes_skipped`. |
...
- **Response 503** `vector_unavailable` (#5068, narrowed by #5069): returned when
  ...
  **Embed-only trigger (#8148).** `PATCH /indexes/{id}/config {"vector": true}`
  ...
  semantic stage is `pending` or `failed`, the PATCH runs the same C2 embed
  catch-up an off→on toggle runs and answers `components.catch_up_started:
  true`. It stays a no-op (`catch_up_started: false`) once the stage is `ready`
  or a pass is already `in_progress`.
...
Four tiers. Only `EMBEDDING_CACHE`, `BM25_CORPUS_CAP`, `MAX_KG_NODES`, and
`MAX_RESIDENT_INDEXES` are keyed off the tier itself; `MEMORY_LIMIT_MB`,
`MAX_CHUNKS`, and `MAX_BATCH_SIZE` scale continuously with actual RAM, so the
representative host in each row is one point on a line, not a bucket.
...
`TRUSTY_MAX_RESIDENT_INDEXES` disabled residency entirely. A host that wants the
old posture sets `TRUSTY_MAX_RESIDENT_INDEXES=off`; a host that already sets a
number is unaffected. The same number also caps eager warm-boot when
`TRUSTY_WARMBOOT_MAX_INDEXES` is unset, so the boot never reaches a peak the
sweep would then have to park back down.
