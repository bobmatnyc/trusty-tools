...
- `skip_vector` flag + runtime KG/vector component toggle (issue #2984 Phase 1): `skip_vector` mirrors `skip_kg` end-to-end (`POST /indexes`, `indexes.toml`, warm-boot stage classification, the reindex pipeline's embed gates) so an index can suppress the vector/semantic lane entirely — the "KG-on, vector-off" quadrant. `PATCH /indexes/:id/config` gains `kg`/`vector` boolean fields to soft-toggle each component at runtime on a live index without a reindex or recreation: turning a component OFF is instantaneous (in-memory KG dropped / embedder stops being invoked; on-disk data untouched); turning one ON spawns a background catch-up (KG: cheap `load_from_corpus`, falling back to a full rebuild; vector: the generalised issue #923 deferred-embed pass, now genuinely incremental — it re-embeds only chunks the vector store doesn't already have, via `VectorStore::contains_many`, never the whole corpus). A code-critic fix round on the same PR closed four gaps before merge: (1) `CodeIndexer` now carries its own `skip_kg` mirror kept in sync on every toggle (`CodeIndexer::set_skip_kg`), fixing a re-enable that previously settled the graph stage `Ready` while the in-memory graph silently stayed empty; (2) the concurrency guard is now a per-`IndexId` semaphore (`service::reindex::index_semaphore`) instead of the process-global `background_reindex_semaphore`, so a toggle on one index is never falsely `409`'d by an unrelated index's background embed, while a genuinely concurrent reindex or catch-up on the SAME index still correctly conflicts; (3) the vector catch-up is incremental as described above; (4) a component disable that lands while a catch-up/deferred-embed pass is already in flight now always wins — the pass's own completion can no longer clobber a `Skipped` stage back to `Ready`. `GET /indexes/:id/status` and `GET /health` surface per-component on/off + catch-up-in-progress state.
...
- bounded cross-project search fan-out + a `--serial` safety valve, fixing the concurrency-limiter 503 storm (issue #2845). The global `POST /search` (and `search_all` MCP) handler previously issued one per-index query per registered index via an **unbounded** `join_all` — a fan-out over ~150+ repo indexes fired ~150 per-index searches near-simultaneously, overran the daemon's admission limiter, and returned `503 server_busy` for the whole request (a peak-~8,000/min storm in production 2026-07-16, causing PR reviews to skip with "no context"). The fan-out now runs with **bounded concurrency** (`buffer_unordered`), so a large fan-out drains in bounded waves and still returns merged results instead of tripping the limiter. The cap is configurable: `trusty-search start --fanout-concurrency N` / the `TRUSTY_SEARCH_FANOUT_CONCURRENCY` env var (default **8**, mirroring `TRUSTY_MAX_CONCURRENT_REQUESTS`), `trusty-search start --serial` (== concurrency 1) as a safety valve for memory/CPU-constrained hosts, and per-request `max_fanout_concurrency` / `serial` body fields (echoed back as `fanout_concurrency` in the response) so a caller can force serial for a single call without restarting the daemon. The `--serial` / `--fanout-concurrency` selection is persisted to `daemon.env` so launchd/systemd restarts keep bounding the fan-out ([#2845](https://github.com/bobmatnyc/trusty-tools/issues/2845))

### Fixed

- issue #2984 Phase 1 delta re-review fix round (PR #3024): (1) HIGH — a chunk edited while `skip_vector=true` whose positional id (`{path}:{start}:{end}`) stayed the same (e.g. a rename or same-line-count edit) previously kept serving its STALE pre-edit vector forever on vector re-enable, because `embed_deferred_chunks`'s `store.contains_many` "needs-embedding" check can't distinguish a genuinely-embedded chunk from one whose id merely collides with stale data. `commit_vectors_batch` now evicts any pre-existing vector-store entry the moment a chunk is committed WITHOUT a fresh embedding (vector lane disabled via `skip_vector`/`lexical_only`/`defer_embed`), restoring the invariant "id present in the store ⇒ vector matches current content" unconditionally; (2) MEDIUM — the per-index reindex/catch-up mutual-exclusion semaphore registry (`INDEX_LOCKS`) never shrank, leaking one `Arc<Semaphore>` per deleted index for the daemon's lifetime; `DELETE /indexes/:id` (and the orphan-reaper's shared `unregister_index` path) now evicts the entry unconditionally — safe because an in-flight task already holds its own `Arc` clone, unaffected by the registry removal; (3) LOW — corrected a stale doc comment on `run_embed_catch_up` that claimed re-running "re-embeds all chunks"; it now correctly says "re-embeds all not-yet-embedded chunks", matching the incremental catch-up behavior
- `embedder_supervisor::LazyEmbedderHandle`'s idle-shutdown watchdog now stops the sidecar via `trusty_common::embedder_client::SupervisorHandle::shutdown()` (cooperative) instead of sending a raw SIGTERM/SIGKILL to its OS PID directly — the raw kill raced the supervisor's crash-restart loop, which had no way to distinguish an intentional idle-shutdown from a crash, and could respawn the sidecar the watchdog had just stopped ([#2979](https://github.com/bobmatnyc/trusty-tools/issues/2979))
...
- warm-boot now honors `skip_kg`: a `skip_kg=true` index previously loaded — or, on a first/never-persisted graph, fully rebuilt — its symbol graph into memory on every daemon restart regardless of the flag, because `IndexHandle::skip_kg` never reached the `CodeIndexer` that owns the actual load/rebuild call. `CodeIndexer` now carries its own `skip_kg` mirror (set by `build_indexer_from_entry` from the persisted entry), and `load_or_rebuild_symbol_graph` checks it before touching the corpus at all — neither the persisted-graph load nor the from-chunks rebuild runs for a skip_kg index, so the ~50-100 MB/index petgraph heap cost the flag was meant to avoid now actually survives a restart. Pure soft-off: on-disk KG tables are untouched; query-time degradation (`search_capabilities` excludes `kg`, `call_chain`/`search_kg` 503 `kg_unavailable`) is unchanged. A code-critic follow-up review of the same PR caught a second door to the identical bug: `create_index_handler` (`POST /indexes`, the re-register path) computed the real `skip_kg` value AFTER already building the indexer via `Default::default()` (`skip_kg = false`), so re-registering an existing id with `skip_kg=true` still loaded its persisted graph during that same call. `skip_kg` is now computed before `init_entry` is constructed and set on the entry so `build_indexer_from_entry` honors it on the create/re-register door the same way it does on daemon-restart warm-boot (Phase 0 of #2984)
- `embedder_supervisor::SupervisorConfig`: new `wedge_reset_secs` field, read from `TRUSTY_EMBEDDERD_WEDGE_RESET_SECS` (default 300s) in `from_env()` and threaded into both `trusty_common::embedder_client::SupervisorConfig` construction sites (`do_spawn`'s production spawn path and `into_common_for_tests`) — the env var previously had no effect on the running daemon because both sites built the common config with `..SupervisorConfig::default()`, silently discarding any operator override. A wedged-but-alive sidecar's restart-storm escalation counter (trusty-common #1448/#1450) is now genuinely tunable without recompiling ([#1448](https://github.com/bobmatnyc/trusty-tools/issues/1448), [#1450](https://github.com/bobmatnyc/trusty-tools/issues/1450))
- LaunchAgent plist now sets an explicit 8192 fd_limit (matching the trusty-memory convention) so fresh installs warm-boot large index fleets without hitting launchd's default 256-fd ceiling; existing installs must re-run `trusty-search service install` to regenerate the plist (closes #2947)
- shutdown-flush no longer truncates HNSW on timeout: `UsearchStore::save` now serializes concurrent savers of the same store, the graceful-shutdown flush deadline scales with each index's on-disk snapshot size (was a flat 10s) and flushes indexes with bounded concurrency instead of strictly sequentially, and a truncated/corrupt on-disk HNSW snapshot is now detected on load (via a pre-`view()` size floor plus a post-view zero-vector-vs-populated-sidecar guard) so `/health` no longer reports semantic search ready when it silently fell back to BM25-only (closes #2922) ([#2930](https://github.com/bobmatnyc/trusty-tools/pull/2930)) ([`f212b2e`](https://github.com/bobmatnyc/trusty-tools/commit/f212b2e1a1e8c74e29a6cf407023599ae3cb53bf))
