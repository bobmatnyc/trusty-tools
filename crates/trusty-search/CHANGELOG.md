...
- `embedder_supervisor::SupervisorConfig`: new `wedge_reset_secs` field (env `TRUSTY_EMBEDDERD_WEDGE_RESET_SECS`, default 300s) forwarded to `trusty_common::embedder_client::SupervisorConfig` so a wedged-but-alive sidecar's restart-storm escalation counter (trusty-common #1448/#1450) is tunable without recompiling ([#1448](https://github.com/bobmatnyc/trusty-tools/issues/1448), [#1450](https://github.com/bobmatnyc/trusty-tools/issues/1450))
