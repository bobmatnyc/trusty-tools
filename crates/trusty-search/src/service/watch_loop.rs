            ...
            match event {
                ...
                WatchEvent::Rescan => {
                    ...
                    file_events
                        ...
                        .await;
                    // #7434: every root, not just this watch's own — see this
                    // function's doc comment for why a per-root pass would
                    // sweep the other roots' chunks out of the corpus.
                    let outcome = crate::service::watch_rescan::reconcile_after_rescan_roots(
                        &index_id,
                        &table,
                        &indexer,
                        &indexed_files,
                ...
                WatchEvent::Modified(path) => {
                    ...
                    handle_modified_in_root(&path, &index_id, &watched, &indexer, &indexed_files)
                        .await;
                }
