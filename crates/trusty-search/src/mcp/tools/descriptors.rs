...
pub fn tool_descriptors() -> Value {
    let mut defs = serde_json::json!([
        ...
        {
            "name": "search",
            "description": "Unified hybrid search (BM25+vector+KG+RRF) with mode-aware ranking (issue #77). The `mode` parameter (\"code\" | \"text\" | \"data\", default \"code\") picks the file-type penalty matrix: code prefers source (prose 0.1x, data 0.2x); text prefers prose docs (source 0.5x, data 0.3x); data prefers structured data (source 0.3x, prose 0.3x). Set `exclude_archived: true` to drop archived/deprecated/legacy chunks entirely instead of downranking them (issue #74). Supports branch-aware scoring via branch_files/branch_boost/branch (issue #122). Exact-match floor (issue #7675): when the query names a literal — an explicitly quoted string, a single identifier carrying an underscore, `::` or a camelCase boundary, a bare filename like `session_mcp_scope.rs`, or an issue reference like `#7675` — every chunk carrying that literal is ranked above every chunk that does not, declaration first and live before archived, and the semantic lanes only order the remainder. An unquoted multi-word phrase is a conceptual query and earns no floor; quote it to ask for it literally. `meta.exact_match_floor` reports whether the floor applied and `meta.exact_match_literal` the text it matched, while `meta.exact_match_degraded` says the lane could not run at all — an absent floor never means the literal is absent. A query naming no literal, or one that occurs nowhere in the corpus, ranks exactly as before. Replaces the legacy `search_code` tool name; callers that omit `mode` get identical pre-#77 behaviour.",
            "inputSchema": {
                "type": "object",
                "required": ["index_id", "query"],
                "properties": {
