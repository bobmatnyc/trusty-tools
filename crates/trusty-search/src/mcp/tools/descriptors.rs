...
pub fn tool_descriptors() -> Value {
    let mut defs = serde_json::json!([
        ...
        {
            "name": "search",
            "description": "Unified hybrid search (BM25+vector+KG+RRF) with mode-aware ranking (issue #77). The `mode` parameter (\"code\" | \"text\" | \"data\", default \"code\") picks the file-type penalty matrix: code prefers source (prose 0.1x, data 0.2x); text prefers prose docs (source 0.5x, data 0.3x); data prefers structured data (source 0.3x, prose 0.3x). Set `exclude_archived: true` to drop archived/deprecated/legacy chunks entirely instead of downranking them (issue #74). Supports branch-aware scoring via branch_files/branch_boost/branch (issue #122). Exact-match floor (issue #7675): when the query names a literal — a single identifier carrying an underscore, `::` or a camelCase boundary, an explicitly quoted string, or a distinctive multi-word phrase — every chunk containing that literal verbatim is ranked above every chunk that does not, declaration first, and the semantic lanes only order the remainder. `meta.exact_match_floor` reports whether the floor applied and `meta.exact_match_literal` the text it matched; a query naming no literal, or one that occurs nowhere in the corpus, ranks exactly as before. Replaces the legacy `search_code` tool name; callers that omit `mode` get identical pre-#77 behaviour.",
            "inputSchema": {
                "type": "object",
                "required": ["index_id", "query"],
                "properties": {
