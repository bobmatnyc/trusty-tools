...
- Agentic detection reads operator-supplied markers from a YAML file, so a target
  ...
  - Operator markers are appended to the shipped set, so they can only classify commits
    it left unmarked — never relabel one it already catches.
  - A marker file that cannot be read, parsed, or compiled is rejected whole, logged at
    `warn!`, and named in `detection_disclosure()`; the run continues on the builtin markers.
