...
pub enum PlanAction {
    ...
    Move,
    /// Frontmatter could not be parsed — left UNTOUCHED and reported. Fail-open
    /// per the conversion contract: one bad file never aborts the whole run.
    Malformed,
}
...
pub struct ConvertReport {
    ...
    /// Root-relative paths of files whose frontmatter failed to parse — left
    /// untouched, never converted (fail-open).
...
impl KbStore {
    ...
    pub fn convert_tree(&self, report_only: bool, now: &str) -> anyhow::Result<ConvertReport> {
        ...
        let unmapped: Vec<String> = plan
            ...
            .collect();
        let malformed: Vec<String> = plan
            .iter()
            .filter(|p| p.action == PlanAction::Malformed)
            .map(|p| p.source.clone())
            .collect();
    ...
    fn plan_concept(
        ...
        // Fail-open: a file whose frontmatter cannot be parsed is reported and
        // left UNTOUCHED — it must never abort the whole conversion (code-critic
        // HIGH). `read_entity_at` errors only on unparseable YAML.
        let entity = match self.read_entity_at(path) {
            Ok(Some(e)) => e,
            Ok(None) => Entity::empty(),
            Err(_) => {
                return Ok(PlanItem {
                    source: rel.to_string(),
                    target: rel.to_string(),
                    assigned_type: "Note".to_string(),
                    action: PlanAction::Malformed,
                    collection: None,
                });
            }
        };
    ...
    fn apply_plan(&self, plan: &[PlanItem], now: &str) -> anyhow::Result<()> {
        ...
        for item in plan {
            // Reserved files are regenerated by ensure_structure; malformed
            // files are left exactly as they are (fail-open — never rewritten).
            if matches!(item.action, PlanAction::Reserved | PlanAction::Malformed) {
                continue;
            }
...
mod tests {
    ...
    /// Why: code-critic HIGH — one malformed-YAML file must not abort the whole
    /// convert. report_only must return a full plan; in_place must convert the
    /// good file, report the bad one, and leave it byte-untouched.
    /// What: a tree with one good + one malformed file; asserts both modes.
    /// Test: self-contained.
    #[test]
    fn convert_fail_open_on_malformed() {
        let (_t, s) = store_with(&[
            (
                "people/ada.md",
                "---\ntype: Person\n---\n\nMathematician.\n",
        ...
        let broken_before = std::fs::read_to_string(s.root.join("people/broken.md")).unwrap();

        // report_only: full plan, broken flagged, nothing written.
        let report = s.convert_tree(true, "2026-01-01T00:00:00Z").unwrap();
        assert!(report.malformed.iter().any(|p| p.ends_with("broken.md")));
        assert!(
            report
                .plan
                .iter()
                .any(|p| p.action == PlanAction::Malformed)
        );
        assert!(report.plan.iter().any(|p| p.source.ends_with("ada.md")));

        // in_place: good file converted, broken left byte-identical.
        s.convert_tree(false, "2026-01-01T00:00:00Z").unwrap();
        let ada =
            Entity::from_content(&std::fs::read_to_string(s.root.join("people/ada.md")).unwrap())
                .unwrap();
        assert_eq!(ada.get_str("type"), Some("Person"));
        assert_eq!(
            std::fs::read_to_string(s.root.join("people/broken.md")).unwrap(),
            broken_before,
