...
/// seed's public half. On `Err`, neither file is left behind: an engagement
/// whose retained public half was never written cannot verify anything it
/// signs, so a half-written mint is removed rather than reported.
///
...
/// publish both files. The config goes through
/// [`crate::workdir::write_private_atomically`], so it is owner-only from the
/// moment it exists and a symlink pre-planted at the temporary's name is
/// refused rather than followed (#5868).
/// Test: `engagement_tests`.
///
/// # Errors
///
...
/// [`AuditError::EngagementNotCreated`] when either file cannot be written.
pub fn create(
    template_path: &Path,
    ...
    // replaced signing key they can no longer reproduce.
    if !options.force {
    ...
    workdir::write_private_atomically(&config_path, &rendered).map_err(|source| {
        AuditError::EngagementNotCreated {
            path: config_path.clone(),
            source: Box::new(source),
        }
    })?;
    // The public half is not a secret and is useless without the package it
    // checks, so it takes the ordinary writer. A trailing newline because the
    // file is read by `verify --public-key`, which trims, and by an operator's
    // `cat`, which does not.
    if let Err(source) =
        workdir::write_atomically(&public_key_path, &format!("{}\n", signing_key.public_hex()))
    {
        // #5478: the config that was just written holds a seed whose public
        // half nobody retained, so nothing could ever verify what it signs.
        // Removing it is what keeps the failure from looking like a success on
        // the next `ls`. The removal is best-effort because the error being
        // returned is the one that matters; it cannot turn this into an `Ok`.
        let _ = std::fs::remove_file(&config_path);
        return Err(AuditError::EngagementNotCreated {
