...
//! [`CATEGORY`], and a roll-up the run index renders ([`Rollup`],
//! [`index_section`]).
//!
//! ## Opt-in, and why
//!
...
/// error count, one count per [`Severity`], and the worst [`TOP_ITEMS`] rows.
...
pub struct Rollup {
    ...
    /// The worst advisories, worst first, capped at [`TOP_ITEMS`].
    pub top: Vec<TopItem>,
}
