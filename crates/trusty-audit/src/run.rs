...
/// artifacts [`verify_output`] requires. On `Err`, the checkpoint records every
/// repository that finished before the failure and is marked incomplete; no
/// claim is made about the rest.
///
    ...
    F: Fn(&str) -> Option<String>,
{
    ...
    for ((index, repo), carried) in selected.into_iter().enumerate().zip(plan) {
        ...
        // timeout or a Ctrl-G after this point costs the repositories still to
        // come and none of the ones already done.
        checkpoint::write_progress(work, &RunProgress::checkpoint(&runs))?;
    }
...
"#;
    ...
    ///
    /// Against the pre-fix behaviour — the record written once, after every
    /// child — the sweep also ends in `Err`, but only after BOTH children have
    /// run. The invocation count is what separates the two.
    ...
    async fn a_checkpoint_that_cannot_be_written_stops_the_sweep() {
        ...
        let (recorder, progress) = Recorder::new();
        let err = sweep(&work, &config(), &RunOptions::default(), &progress)
            .await
    ...
    async fn a_re_run_skips_what_succeeded_and_retries_what_failed() {
        ...
        let ran = invocations(&work);
        assert_eq!(ran.len(), 3, "only the failed repository may run again: {ran:?}");
    ...
    async fn a_fresh_run_re_audits_everything() {
        ...
        .await
        .expect("the sweep completes");
        assert_eq!(invocations(&work).len(), 2, "`--fresh` must run the child again");
